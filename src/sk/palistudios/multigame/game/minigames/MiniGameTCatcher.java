...
//        difficultyStep = 8;
    ...
    public void drawMinigame(Canvas mCanvas) {
        ...
        for (int i = 0; i < numberOfColumns; i++) {
            if (i != activeBall) {
                mCanvas.drawCircle(mCatchingBalls[i], catchingBallsHeight, mBallSize, mPaintCatchingBallInactive.mPaint);
            } else {
                mCanvas.drawCircle(mCatchingBalls[i], catchingBallsHeight, mBallSize, mPaintCatchingBallActive.mPaint);
            }
        ...
        private boolean isCatched() {
            ...
            if (yAxis + mBallSize + 1 > mHeight) {
                ...
                if (mColumn == activeBall) {
                    if (yAxis + mBallSize <= catchingBallsHeight + mBallSize && yAxis + mBallSize >= catchingBallsHeight - mBallSize) {
                        return true;
                    }
                    if (yAxis - mBallSize <= catchingBallsHeight + mBallSize && yAxis - mBallSize >= catchingBallsHeight - mBallSize) {
                        return true;
