...
//        difficultyStep = 8;
    ...
    public void onUserInteracted(float x, float y) {
        ...
        for (CircleToTouch obj : mCircles) {
            if ((touchX < obj.x && touchX > obj.x - touchingDistance) || (touchX > obj.x && touchX < obj.x + touchingDistance)) {
                if ((touchY < obj.y && touchY > obj.y - touchingDistance) || (touchY > obj.y && touchY < obj.y + touchingDistance)) {
    ...
    private void generateNewObjects() {
        ...
        if (framesToGo == 0) {
            CircleToTouch circle = new CircleToTouch(mRg.generateInt(0 + (mCircleSize), mWidth - (mCircleSize)), mRg.generateInt(0 + (mCircleSize), mHeight - (mCircleSize)), 9);
...
//        return null;
    ...
    private boolean colidesWithOtherCircles(CircleToTouch circleNew) {
        for (CircleToTouch circleOld : mCircles) {
            if (circleNew.x > circleOld.x - 2 * mCircleSize && circleNew.x < circleOld.x + 2 * mCircleSize) {
                if (circleNew.y > circleOld.y - 2 * mCircleSize && circleNew.y < circleOld.y + 2 * mCircleSize) {
                    return true;
                }
            }
        }
