        ...
        getListView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (musicArrayAdapter.getItem(position - 1).isLocked()) {
                    Toaster.toastLong(musicArrayAdapter.getItem(position - 1).getLockedDescription(), getParent());
                    return;
                }
...
//        items.add(new MusicItem("abdessamie_beat", "Abdessamie beat", GameSharedPref.isMusicLoopChosen("abdessamie_beat")));
//        items.add(new MusicItem("jungle", "Jungle", GameSharedPref.isMusicLoopChosen("jungle"), (String) getResources().getString(R.string.cc_music_unlock_message_1)));
        items.add(new MusicItem("dst_blam", "Blam", GameSharedPref.isMusicLoopChosen("dst_blam")));//, "You must reach level 10 in order to unlock this music."));
        items.add(new MusicItem("dst_cv_x", "CV X", GameSharedPref.isMusicLoopChosen("dst_cv_x"), (String) getResources().getString(R.string.cc_achievements_supporter_description) + (String) getResources().getString(R.string.cc_achievements_requirement_ending)));//, "You must reach level 10 in order to unlock this music."));
        items.add(new MusicItem("dst_cyberops", "Cyber Ops", GameSharedPref.isMusicLoopChosen("dst_cyberops"), (String) getResources().getString(R.string.cc_achievements_lucky_seven_description) + (String) getResources().getString(R.string.cc_achievements_requirement_ending)));//, "You must reach level 10 in order to unlock this music."));
...
//        }
        ...
        musicArrayAdapter = new MusicArrayAdapter(this, items, SkinsCenterListActivity.getCurrentSkin(this));
    }
