...
public final class Settings {
  ...
  private static final Uri ATTRIBUTION_ID_CONTENT_URI = Uri.parse(
   ...
   * production. <p/> Returns the types of extended logging that are currently enabled.
   *
  ...
  /**
   * Certain logging behaviors are available for debugging beyond those that should be enabled in
   * production. <p/> Enables a particular extended logging in the sdk.
   *
  ...
  /**
   * Certain logging behaviors are available for debugging beyond those that should be enabled in
   * production. <p/> Disables a particular extended logging behavior in the sdk.
   *
  ...
  /**
   * Certain logging behaviors are available for debugging beyond those that should be enabled in
   * production. <p/> Disables all extended logging behaviors.
   */
  ...
  /**
   * Certain logging behaviors are available for debugging beyond those that should be enabled in
   * production. <p/> Checks if a particular extended logging behavior is enabled.
   *
  ...
  /**
   * Returns the Executor used by the SDK for non-AsyncTask background work. <p/> By default this
   * uses AsyncTask Executor via reflection if the API level is high enough. Otherwise this
   * creates
   * a new Executor with defaults similar to those used in AsyncTask.
   *
  ...
  /**
   * Sets the base Facebook domain to use when making Web requests. This defaults to "facebook
   * .com",
   ...
   * @param applicationId the fb application being published. <p/> This method is deprecated. See
   ...
   * @param callback a callback to invoke with a Response object, carrying the server response, or
   * an error. <p/> This method is deprecated. See {@link AppEventsLogger#activateApp(Context,
  ...
  /**
   * Gets whether opening a Session should automatically publish install attribution to the
   * Facebook
   * graph.
   *
   * @return true to automatically publish, false to not <p/> This method is deprecated. See {@link
   * AppEventsLogger#activateApp(Context, String)} for more info.
   */
  @Deprecated
  ...
  /**
   * Sets whether opening a Session should automatically publish install attribution to the
   * Facebook
   * graph.
   *
   * @param shouldAutoPublishInstall true to automatically publish, false to not <p/> This method is
   * deprecated. See {@link AppEventsLogger#activateApp(Context, String)} for more info.
   */
  @Deprecated
   ...
   * again after true is returned. <p/> This method is deprecated. See {@link
   * AppEventsLogger#activateApp(Context, String)} for more info.
   */
  @Deprecated
  ...
  /**
   * Manually publish install attribution to the Facebook graph. Internally handles caching repeat
   * calls to prevent multiple installs being published to the graph.
   *
   * @param context the current Context
   * @param applicationId the fb application being published.
   * @return returns a Response object, carrying the server response, or an error. <p/> This method
   * is deprecated. See {@link AppEventsLogger#activateApp(Context, String)} for more info.
   */
  @Deprecated
  public static Response publishInstallAndWaitForResponse(final Context context,
  ...
  static Response publishInstallAndWaitForResponse(final Context context,
    ...
    try {
      ...
      publishParams.setProperty("application_tracking_enabled", !AppEventsLogger.getLimitEventUsage(
          context));
      ...
      if (lastPing != 0) {
        ...
        if (graphObject == null) {
          return Response.createResponsesFromString("true", null, new RequestBatch(publishRequest),
              true).get(0);
        } else {
        ...
        if (publishResponse.getGraphObject() != null &&
          ...
          editor.putString(jsonKey,
              publishResponse.getGraphObject().getInnerJSONObject().toString());
        }
        editor.commit();
  ...
  /**
   * Sets the application version to the provided string. AppEventsLogger.logEvent calls logs its
   * event with the current app version, and App Insights allows breakdown of events by app
   * version.
   *
   * @param appVersion The version identifier of the Android app that events are being logged
   * through. Enables analysis and breakdown of logged events by app version.
   */
   ...
   * endpoints to specify a set of platform migrations that are explicitly turned on or off for that
   * call, in order to ensure compatibility between a given version of the SDK and the Graph API.
   *
