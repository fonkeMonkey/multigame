...
/**
 * This class represents an access token returned by the Facebook Login service, along with
 * associated metadata such as its expiration date and permissions. In general, the {@link Session}
 * class will abstract away the need to worry about the details of an access token, but there are
 * situations (such as handling native links, importing previously-obtained access tokens, etc.)
 * where it is useful to deal with access tokens directly. Factory methods are provided to construct
 * access tokens. <p/> For more information on access tokens, see <a
...
public final class AccessToken implements Serializable {
  ...
  AccessToken(String token, Date expires, List<String> permissions, AccessTokenSource source,
      Date lastRefresh) {
    if (permissions == null) {
   ...
   * token (for instance, from an already-cached access token obtained prior to integration
   * with the
   * Facebook SDK).
   *
   ...
   * @param permissions the permissions that were requested when the token was obtained (or
   * when it
  ...
  public static AccessToken createFromExistingAccessToken(String accessToken, Date expirationTime,
    ...
    return new AccessToken(accessToken, expirationTime, permissions, accessTokenSource,
        lastRefreshTime);
  }
   ...
   * linking,
   * please see https://developers.facebook.com/docs/mobile/android/deep_linking/.
   *
  ...
  public static AccessToken createFromNativeLinkingIntent(Intent intent) {
    ...
    return createFromBundle(null, intent.getExtras(), AccessTokenSource.FACEBOOK_APPLICATION_WEB,
        new Date());
  }
  ...
  static AccessToken createFromString(String token, List<String> permissions,
    ...
    return new AccessToken(token, DEFAULT_EXPIRATION_TIME, permissions, source,
        DEFAULT_LAST_REFRESH_TIME);
  }

  static AccessToken createFromNativeLogin(Bundle bundle, AccessTokenSource source) {
    Date expires = getBundleLongAsDate(bundle, NativeProtocol.EXTRA_EXPIRES_SECONDS_SINCE_EPOCH,
        new Date(0));
  ...
  static AccessToken createFromWebBundle(List<String> requestedPermissions, Bundle bundle,
      AccessTokenSource source) {
  ...
  static AccessToken createFromTokenWithRefreshedPermissions(AccessToken token,
      List<String> permissions) {
    return new AccessToken(token.token, token.expires, permissions, token.source,
        token.lastRefresh);
  }

  private static AccessToken createNew(List<String> requestedPermissions, String accessToken,
      Date expires, AccessTokenSource source) {
  ...
  static AccessToken createFromCache(Bundle bundle) {
    ...
    List<String> originalPermissions = bundle.getStringArrayList(
    ...
    return new AccessToken(bundle.getString(TokenCachingStrategy.TOKEN_KEY),
        TokenCachingStrategy.getDate(bundle, TokenCachingStrategy.EXPIRATION_DATE_KEY), permissions,
   ...
   * more information on permissions, see https://developers.facebook
   * .com/docs/reference/login/#permissions.
   *
  ...
  Bundle toCacheBundle() {
    ...
    bundle.putStringArrayList(TokenCachingStrategy.PERMISSIONS_KEY, new ArrayList<String>(
        permissions));
  ...
  private static class SerializationProxyV1 implements Serializable {
    ...
    private SerializationProxyV1(String token, Date expires, List<String> permissions,
