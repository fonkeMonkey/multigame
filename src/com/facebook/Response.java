...
public class Response {
   ...
   * non-JSON response (e.g., the string literal "true" or "false"). To present a consistent
   * way of
   * accessing results, these are represented as a GraphObject with a single string property with
   * this name.
   */
  ...
  Response(Request request, HttpURLConnection connection, GraphObject graphObject,
      boolean isFromCache) {
  ...
  static FileLruCache getResponseCache() {
    if (responseCache == null) {
      ...
      if (applicationContext != null) {
        responseCache = new FileLruCache(applicationContext, RESPONSE_CACHE_TAG,
  ...
  static List<Response> fromHttpConnection(HttpURLConnection connection, RequestBatch requests) {
    ...
    if (requests instanceof CacheableRequestBatch) {
      ...
      if (!cacheableRequestBatch.getForceRoundTrip() && cache != null && !Utility.isNullOrEmpty(
          cacheKey)) {
        try {
    ...
    try {
      ...
      Logger.log(LoggingBehavior.REQUESTS, RESPONSE_LOG_TAG, "Response <Error>: %s",
          facebookException);
  ...
  static List<Response> createResponsesFromStream(InputStream stream, HttpURLConnection connection,
      RequestBatch requests, boolean isFromCache)
  ...
  static List<Response> createResponsesFromString(String responseString,
      HttpURLConnection connection, RequestBatch requests, boolean isFromCache)
      throws FacebookException, JSONException, IOException {
  ...
  private static List<Response> createResponsesFromObject(HttpURLConnection connection,
      List<Request> requests, Object object, boolean isFromCache)
    ...
    if (numRequests == 1) {
      ...
      try {
        // Single request case -- the entire response is the result, wrap it as "body" so we can
        // handle it
        // the same as we do in the batched case. We get the response code from the actual HTTP
        // response,
  ...
  private static Response createResponseFromObject(Request request, HttpURLConnection connection,
      Object object, boolean isFromCache, Object originalResult) throws JSONException {
  ...
  static List<Response> constructErrorResponses(List<Request> requests,
    ...
    for (int i = 0; i < count; ++i) {
      Response response = new Response(requests.get(i), connection, new FacebookRequestError(
          connection, error));
  ...
  /**
   * The single graph object returned for this request, if any, cast into a particular type of
   * GraphObject.
   *
   ...
   *
   * @return the list of graph objects returned, or null if none was returned (or if the result was
   * not a list)
   */
  ...
  /**
   * The list of graph objects returned for this request, if any, cast into a particular type of
   * GraphObject.
   *
   * @param graphObjectClass the GraphObject-derived interface to cast the graph objects into
   * @return the list of graph objects returned, or null if none was returned (or if the result
   * was
   * not a list)
   * @throws FacebookException If the passed in Class is not a valid GraphObject interface
   */
  ...
  /**
   * If a Response contains results that contain paging information,
   * returns a new Request that will
   * retrieve the next page of results, in whichever direction is desired. If no paging
   * information
  ...
  public String toString() {
    ...
    return new StringBuilder().append("{Response: ").append(" responseCode: ").append(responseCode)
        .append(", graphObject: ").append(graphObject).append(", error: ").append(error).append(
            ", isFromCache:").append(isFromCache).append("}").toString();
  }
