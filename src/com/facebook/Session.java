...
/**
 * <p> Session is used to authenticate a user and manage the user's session with
 * Facebook. </p> <p> Sessions must be opened before they can be used to make a
 * Request. When a Session is created, it attempts to initialize itself from a
 ...
 * must be created. </p> <p> Instances of Session provide state change
 * notification via a callback interface,
 * {@link Session.StatusCallback StatusCallback}. </p>
 */
public class Session implements Serializable {
     ...
     *                        null
     * @param cachingStrategy the TokenCachingStrategy to use to load and store
     *                        the token. If this is null, a default token cachingStrategy that stores
     *                        data in SharedPreferences will be used
     * @param callback        the callback to notify for Session state changes, can be
     *                        null
    ...
    /**
     * <p> Sets the current active Session. </p> <p> The active Session is used
     * implicitly by predefined Request factory methods as well as optionally by
     * UI controls in the sdk. </p> <p> It is legal to set this to null, or to a
     * Session that is not yet open. </p>
     *
     * @param session A Session to use as the active Session, or null to
     *                indicate that there is no active Session.
     */
     ...
     * @param allowLoginUI if false, only sets the active session and opens it
     *                     if it does not require user interaction
     ...
     * @param callback    a callback that will be called when the session status
     *                    changes; may be null
     * @return The new Session or null if one could not be created
     */
    ...
    /**
     * <p> Returns the Date at which the current token will expire. </p> <p>
     * Note that Session automatically attempts to extend the lifetime of Tokens
     * as needed when Facebook requests are made. </p>
     *
    ...
    /**
     * <p> Returns the list of permissions associated with the session. </p> <p>
     ...
     * {@link #requestNewPublishPermissions}. </p>
     *
    ...
    /**
     * <p> Logs a user in to Facebook. </p> <p> A session may not be used with
     * {@link Request Request} and other classes in the SDK until it is open.
     * If, prior to calling open, the session is in the
     * {@link SessionState#CREATED_TOKEN_LOADED CREATED_TOKEN_LOADED} state, and
     * the requested permissions are a subset of the previously authorized
     * permissions, then the Session becomes usable immediately with no user
     * interaction. </p> <p> The permissions associated with the openRequest
     * passed to this method must be read permissions only (or null/empty). It
     * is not allowed to pass publish permissions to this method and will result
     * in an exception being thrown. </p> <p> Any open method must be called at
     * most once, and cannot be called after the Session is closed. Calling the
     * method at an invalid time will result in UnsuportedOperationException.
     * </p>
     *
     * @param openRequest the open request, can be null only if the Session is
     *                    in the {@link SessionState#CREATED_TOKEN_LOADED CREATED_TOKEN_LOADED}
     *                    state
     * @throws FacebookException if any publish or manage permissions are
     *                           requested
     */
     ...
     * permissions, then the Session becomes usable immediately with no user
     * interaction. </p> <p> The permissions associated with the openRequest
     * passed to this method must be publish or manage permissions only and must
     * be non-empty. Any read permissions will result in a warning, and may fail
     * during server-side authorization. Also, an application must have at least
     * basic read permissions prior to requesting publish permissions, so this
     * method should only be used if the application knows that the user has
     * already granted read permissions to the application; otherwise,
     * openForRead should be used, followed by a call to
     ...
     * <p> Any open method must be called at most once, and cannot be called
     * after the Session is closed. Calling the method at an invalid time will
     * result in UnsuportedOperationException. </p>
     *
     * @param openRequest the open request, can be null only if the Session is
     *                    in the {@link SessionState#CREATED_TOKEN_LOADED CREATED_TOKEN_LOADED}
     *                    state
     * @throws FacebookException if the passed in request is null or has no
     *                           permissions set.
     */
    ...
    /**
     * <p> Issues a request to add new read permissions to the Session. </p> <p>
     ...
     * </p> <p> The permissions associated with the newPermissionsRequest passed
     * to this method must be read permissions only (or null/empty). It is not
     * allowed to pass publish permissions to this method and will result in an
    ...
    /**
     * <p> Issues a request to add new publish or manage permissions to the
     * Session. </p> <p> If successful, this will update the set of permissions
     * on this session to match the newPermissions. If this fails, the Session
     * remains unchanged. </p> <p> The permissions associated with the
     * newPermissionsRequest passed to this method must be publish or manage
     * permissions only and must be non-empty. Any read permissions will result
     * in a warning, and may fail during server-side authorization. </p>
     *
     * @param newPermissionsRequest the new permissions request
     */
     ...
     * and data.
     *
     * @param currentActivity The Activity that is forwarding the
    ...
    public static final class OpenRequest extends AuthorizationRequest {
         ...
         *
         * @param statusCallback The
         *                       {@link StatusCallback SessionStatusCallback} to notify regarding
         ...
         *
         * @param requestCode An integer that identifies this request. This
         *                    integer will be used as the request code in {@link Activity#onActivityResult
         ...
         *
         * @param permissions A List&lt;String&gt; representing the permissions
         *                    to request during the authentication flow. A null or empty List
         ...
         * Sets the permissions for the OpenRequest.
         *
         * @param permissions the permissions to request during the
         *                    authentication flow.
         * @return the OpenRequest object to allow for chaining
         */
         ...
         * facebook app.
         *
         * @param defaultAudience A SessionDefaultAudience representing the
         *                        default audience setting to request.
         * @return the OpenRequest object to allow for chaining
         */
