...
public class Session implements Serializable {
  ...
  /**
   * The action used to indicate that the active session has been opened. This should be used
   * as an
   * action in an IntentFilter and BroadcastReceiver registered with the {@link
   * android.support.v4.content.LocalBroadcastManager}.
   */
  ...
  /**
   * The action used to indicate that the active session has been closed. This should be used
   * as an
   * action in an IntentFilter and BroadcastReceiver registered with the {@link
   * android.support.v4.content.LocalBroadcastManager}.
   */
  ...
  /**
   * Session takes application id as a constructor parameter. If this is null, Session will attempt
   * to load the application id from application/meta-data using this String as the key.
   */
  ...
  Session(Context context, String applicationId, TokenCachingStrategy tokenCachingStrategy,
    ...
    if (TokenCachingStrategy.hasTokenInformation(tokenState)) {
      Date cachedExpirationDate = TokenCachingStrategy.getDate(tokenState,
  ...
  /**
   * Save the Session object into the supplied Bundle. This method is intended to be called
   * from an
   * Activity or Fragment's onSaveInstanceState method in order to preserve Sessions across Activity
   * lifecycle events.
   *
   ...
   * @param cachingStrategy the TokenCachingStrategy to use to load and store the token. If
   * this is
   * null, a default token cachingStrategy that stores data in SharedPreferences will be used
   ...
   * predefined Request factory methods as well as optionally by UI controls in the sdk. </p>
   * <p> It
   * is legal to set this to null, or to a Session that is not yet open. </p>
   *
   ...
   * default
   * token cache is available and cached tokens are loaded, this will use the cached token and
   * associated permissions. <p/> If allowedLoginUI is false, this will only create the active
   ...
   * @param allowLoginUI if false, only sets the active session and opens it if it does not require
   * user interaction
   * @param callback The {@link StatusCallback SessionStatusCallback} to notify regarding Session
   * state changes. May be null.
   * @return The new Session or null if one could not be created
   */
  public static Session openActiveSession(Activity activity, boolean allowLoginUI,
      StatusCallback callback) {
    return openActiveSession(activity, allowLoginUI, new OpenRequest(activity).setCallback(
        callback));
  }
  ...
  public static Session openActiveSession(Context context, Fragment fragment, boolean allowLoginUI,
      StatusCallback callback) {
    return openActiveSession(context, allowLoginUI, new OpenRequest(fragment).setCallback(
        callback));
  }

  /**
   * Opens a session based on an existing Facebook access token, and also makes this session the
   * currently active session. This method should be used only in instances where an
   * application has
   * previously obtained an access token and wishes to import it into the
   ...
   * -- after that, the usual Session lifecycle methods should be used to manage the session
   * and its
   * associated token. <p/> No validation is done that the token, token source, or permissions are
   * actually valid. It is the caller's responsibility to ensure that these accurately reflect the
   * state of the token that has been passed in, or calls to the Facebook API may fail.
   *
  ...
  public static boolean isPublishPermission(String permission) {
    return permission != null && (permission.startsWith(PUBLISH_PERMISSION_PREFIX) ||
        permission.startsWith(MANAGE_PERMISSION_PREFIX) || OTHER_PUBLISH_PERMISSIONS.contains(
        permission));
   ...
   * Request}
   * and other classes in the SDK until it is open. If, prior to calling open,
   * the session is in the
   * {@link SessionState#CREATED_TOKEN_LOADED CREATED_TOKEN_LOADED} state, and the requested
   * permissions are a subset of the previously authorized permissions, then the Session becomes
   * usable immediately with no user interaction. </p> <p> The permissions associated with the
   * openRequest passed to this method must be read permissions only (or null/empty). It is not
   * allowed to pass publish permissions to this method and will result in an exception being
   * thrown. </p> <p> Any open method must be called at most once, and cannot be called after the
   ...
   * permissions are a subset of the previously authorized permissions, then the Session becomes
   * usable immediately with no user interaction. </p> <p> The permissions associated with the
   * openRequest passed to this method must be publish or manage permissions only and must be
   * non-empty. Any read permissions will result in a warning, and may fail during server-side
   * authorization. Also, an application must have at least basic read permissions prior to
   * requesting publish permissions, so this method should only be used if the application knows
   * that the user has already granted read permissions to the application; otherwise, openForRead
   * should be used, followed by a call to requestNewPublishPermissions. For more information on
   * this flow, see https://developers.facebook.com/docs/facebook-login/permissions/. </p> <p> Any
   * open method must be called at most once, and cannot be called after the Session is closed.
   * Calling the method at an invalid time will result in UnsuportedOperationException. </p>
   *
   * @param openRequest the open request, can be null only if the Session is in the {@link
   * SessionState#CREATED_TOKEN_LOADED CREATED_TOKEN_LOADED} state
   ...
   * only in
   * instances where an application has previously obtained an access token and wishes to
   * import it
   * into the Session/TokenCachingStrategy-based session-management system. An example would be an
   * application which previously did not use the Facebook SDK for Android and implemented its own
   * session-management scheme, but wishes to implement an upgrade path for existing users so they
   * do not need to log in again when upgrading to a version of the app that uses the SDK. <p/> No
   * validation is done that the token, token source, or permissions are actually valid. It is the
   * caller's responsibility to ensure that these accurately reflect the state of the token that has
   * been passed in, or calls to the Facebook API may fail.
   *
   * @param accessToken the access token obtained from Facebook
   * @param callback a callback that will be called when the session status changes; may be null
   */
   ...
   * fails,
   * the Session remains unchanged. </p> <p> The permissions associated with the
   ...
   * permissions only and must be non-empty. Any read permissions will result in a warning,
   * and may
   * fail during server-side authorization. </p>
   *
   * @param newPermissionsRequest the new permissions request
   */
   ...
   * updates
   * the Session based on information returned during the authorization flow. The Activity that
   * calls open or requestNewPermissions should forward the resulting onActivityResult call
   * here to
   * update the Session state based on the contents of the resultCode and data.
   *
   ...
   * onActivityResult occurs as part of Facebook authorization flow, this value is the activityCode
   * passed to open or authorize.
  ...
  public String toString() {
    return new StringBuilder().append("{Session").append(" state:").append(this.state).append(
        ", token:").append((this.tokenInfo == null) ? "null" : this.tokenInfo).append(", appId:")
  ...
  private Object writeReplace() {
    return new SerializationProxyV1(applicationId, state, tokenInfo, lastAttemptedTokenExtendDate,
        false, pendingAuthorizationRequest);
  }
  ...
  private void requestNewPermissions(NewPermissionsRequest newPermissionsRequest,
    ...
    if (newPermissionsRequest != null) {
      synchronized (this.lock) {
        if (pendingAuthorizationRequest != null) {
          throw new UnsupportedOperationException(
              "Session: an attempt was made to request new permissions for a session that has a " +
                  "pending request.");
        }
        if (state.isOpened()) {
          ...
          throw new UnsupportedOperationException(
              "Session: an attempt was made to request new permissions for a session that has " +
                  "been closed.");
        } else {
          throw new UnsupportedOperationException(
              "Session: an attempt was made to request new permissions for a session that is not " +
                  "currently open.");
        }
      }
    ...
    if (request != null && !request.isLegacy) {
      ...
      if (!resolveIntent(intent)) {
        throw new FacebookException(String.format(
            "Cannot use SessionLoginBehavior %s when %s is not declared as an activity in " +
                "AndroidManifest.xml",
      ...
      if (isPublishPermission(permission)) {
        ...
        if (SessionAuthorizationType.PUBLISH.equals(authType)) {
          Log.w(TAG, String.format(
                  "Should not pass a read permission (%s) to a request for publish or manage " +
                      "authorization",
                  permission));
        }
      }
  ...
  private void logAuthorizationStart() {
    Bundle bundle = AuthorizationClient.newAuthorizationLoggingBundle(
    ...
    try {
      ...
      extras.put(AuthorizationClient.EVENT_EXTRAS_PERMISSIONS, TextUtils.join(",",
          pendingAuthorizationRequest.permissions));
  ...
  void postStateChange(final SessionState oldState, final SessionState newState,
    ...
    // different now.
    if ((oldState == newState) && (oldState != SessionState.OPENED_TOKEN_UPDATED) &&
        (exception == null)) {
      return;
    }
  ...
  public boolean equals(Object otherObj) {
    ...
    return areEqual(other.applicationId, applicationId) && areEqual(other.authorizationBundle,
        authorizationBundle) && areEqual(other.state, state) && areEqual(other.getExpirationDate(),
        getExpirationDate());
  }
  ...
  private static class SerializationProxyV1 implements Serializable {
    ...
    private Object readResolve() {
      return new Session(applicationId, state, tokenInfo, lastAttemptedTokenExtendDate,
          shouldAutoPublish, pendingAuthorizationRequest);
    }
  }
  ...
  public static class AuthorizationRequest implements Serializable {
    ...
    Object writeReplace() {
      return new AuthRequestSerializationProxyV1(loginBehavior, requestCode, permissions,
          defaultAudience.name(), isLegacy, applicationId, validateSameFbidAsToken);
    }

    // have a readObject that throws to prevent spoofing; must be private so serializer will call
    // it (will be
    ...
    private static class AuthRequestSerializationProxyV1 implements Serializable {
      ...
      private Object readResolve() {
        return new AuthorizationRequest(loginBehavior, requestCode, permissions, defaultAudience,
            isLegacy, applicationId, validateSameFbidAsToken);
      }
    }
  ...
  public static final class OpenRequest extends AuthorizationRequest {
     ...
     * as the
     * request code in {@link Activity#onActivityResult onActivityResult}. This integer should be >=
     * 0. If a value < 0 is passed in, then a default value will be used.
     * @return the OpenRequest object to allow for chaining
     */
     ...
     *
     * @param permissions A List&lt;String&gt; representing the permissions to request during the
     * authentication flow. A null or empty List represents basic permissions.
     * @return the OpenRequest object to allow for chaining
     */
     ...
     *
     * @param defaultAudience A SessionDefaultAudience representing the default audience setting to
     * request.
     * @return the OpenRequest object to allow for chaining
     */
  ...
  public static final class NewPermissionsRequest extends AuthorizationRequest {
     ...
     *
     * @param requestCode An integer that identifies this request. This integer will be used as the
     * request code in {@link Activity#onActivityResult onActivityResult}. This integer should be >=
     * 0. If a value < 0 is passed in, then a default value will be used.
     * @return the NewPermissionsRequest object to allow for chaining
     */
  ...
  class TokenRefreshRequest implements ServiceConnection {
    ...
    final Messenger messageReceiver = new Messenger(new TokenRefreshRequestHandler(Session.this,
        this));
