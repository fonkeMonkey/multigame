...
public final class NativeProtocol {
  ...
  public static final String ACTION_LOGIN_DIALOG =
      "com.facebook.platform.action.request.LOGIN_DIALOG";
  ...
  public static final Uri PLATFORM_PROVIDER_VERSIONS_URI = Uri.parse(
      CONTENT_SCHEME + PLATFORM_PROVIDER + "/versions");
  static final String FACEBOOK_PROXY_AUTH_ACTIVITY = "com.facebook.katana.ProxyAuth";
  static final String FACEBOOK_TOKEN_REFRESH_ACTIVITY =
  ...
  public static Intent createProxyAuthIntent(Context context, String applicationId,
      List<String> permissions, String e2e) {
    Intent intent = new Intent().setClassName(FACEBOOK_PACKAGE, FACEBOOK_PROXY_AUTH_ACTIVITY)
  ...
  public static Intent createTokenRefreshIntent(Context context) {
    Intent intent = new Intent().setClassName(FACEBOOK_PACKAGE, FACEBOOK_TOKEN_REFRESH_ACTIVITY);

    return validateKatanaServiceIntent(context, intent);
  }

  public static Intent createPlatformActivityIntent(Context context, String action, int version,
      Bundle extras) {
    Intent intent = new Intent().setAction(INTENT_ACTION_PLATFORM_ACTIVITY).setPackage(
        FACEBOOK_PACKAGE).addCategory(Intent.CATEGORY_DEFAULT).putExtras(extras).putExtra(
        EXTRA_PROTOCOL_VERSION, version).putExtra(EXTRA_PROTOCOL_ACTION, action);
    return validateKatanaActivityIntent(context, intent);
  }

  public static Intent createPlatformServiceIntent(Context context) {
    Intent intent = new Intent(INTENT_ACTION_PLATFORM_SERVICE).setPackage(FACEBOOK_PACKAGE)
        .addCategory(Intent.CATEGORY_DEFAULT);
    return validateKatanaServiceIntent(context, intent);
  }

  public static Intent createLoginDialog20121101Intent(Context context, String applicationId,
      ArrayList<String> permissions, String audience) {
    Intent intent = new Intent().setAction(INTENT_ACTION_PLATFORM_ACTIVITY).setPackage(
        FACEBOOK_PACKAGE).addCategory(Intent.CATEGORY_DEFAULT).putExtra(EXTRA_PROTOCOL_VERSION,
        PROTOCOL_VERSION_20121101).putExtra(EXTRA_PROTOCOL_ACTION, ACTION_LOGIN_DIALOG).putExtra(
        EXTRA_APPLICATION_ID, applicationId).putStringArrayListExtra(EXTRA_PERMISSIONS,
        ensureDefaultPermissions(permissions)).putExtra(EXTRA_PROTOCOL_CALL_ID, generateCallId())
  ...
  public static boolean isServiceDisabledResult20121101(Intent data) {
    ...
    return ((PROTOCOL_VERSION_20121101 == protocolVersion) && ERROR_SERVICE_DISABLED.equals(
        errorType));
  }
