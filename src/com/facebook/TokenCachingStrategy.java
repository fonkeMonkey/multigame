...
/**
 * <p> A base class for implementations of a {@link Session Session} token
 * cache. </p> <p> The Session constructor optionally takes a
 * TokenCachingStrategy, from which it will attempt to load a cached token
 * during construction. Also, whenever the Session updates its token, it will
 * also save the token and associated state to the TokenCachingStrategy. </p>
 * <p> This is the only mechanism supported for an Android service to use
 * Session. The service can create a custom TokenCachingStrategy that returns
 * the Session provided by an Activity through which the user logged in to
 * Facebook. </p>
 */
public abstract class TokenCachingStrategy {
    ...
    /**
     * The key used by Session to store an enum indicating the source of the
