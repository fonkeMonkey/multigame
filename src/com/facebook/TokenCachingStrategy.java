...
/**
 * <p> A base class for implementations of a {@link Session Session} token cache. </p> <p> The
 * Session constructor optionally takes a TokenCachingStrategy, from which it will attempt to load a
 * cached token during construction. Also, whenever the Session updates its token, it will also save
 * the token and associated state to the TokenCachingStrategy. </p> <p> This is the only mechanism
 * supported for an Android service to use Session. The service can create a custom
 * TokenCachingStrategy that returns the Session provided by an Activity through which the user
...
public abstract class TokenCachingStrategy {
  ...
  /**
   * The key used by Session to store the expiration date value in the Bundle during load and
   * save.
   */
  public static final String EXPIRATION_DATE_KEY =
      "com.facebook.TokenCachingStrategy.ExpirationDate";
  /**
   * The key used by Session to store the last refresh date value in the Bundle during load and
   * save.
   */
  ...
  /**
   * The key used by Session to store an enum indicating the source of the token in the Bundle
   * during load and save.
   */
  public static final String TOKEN_SOURCE_KEY =
      "com.facebook.TokenCachingStrategy" + ".AccessTokenSource";
  /**
   * The key used by Session to store the list of permissions granted by the token in the Bundle
   * during load and save.
   */
  ...
  /**
   * Called during Session construction to get the token state. Typically this is loaded from a
   * persistent store that was previously initialized via save. The caller may choose to keep a
   * reference to the returned Bundle indefinitely. Therefore the TokenCachingStrategy should not
   * store the returned Bundle and should return a new Bundle on every call to this method.
   *
  ...
  /**
   * Called when a Session updates its token. This is passed a Bundle of values that should be
   * stored durably for the purpose of being returned from a later call to load. Some
   * implementations may choose to store bundle beyond the scope of this call, so the caller
   * should
   * keep no references to the bundle to ensure that it is not modified later.
   *
  ...
  /**
   * Called when a Session learns its token is no longer valid or during a call to {@link
   * Session#closeAndClearTokenInformation closeAndClearTokenInformation} to clear the durable state
