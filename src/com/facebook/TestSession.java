...
/**
 * Implements an subclass of Session that knows about test users for a particular application. This
 * should never be used from a real application, but may be useful for writing unit tests, etc. <p/>
 * Facebook allows developers to create test accounts for testing their applications' Facebook
 * integration (see https://developers.facebook.com/docs/test_users/). This class simplifies use of
 * these accounts for writing unit tests. It is not designed for use in production application code.
 * <p/> The main use case for this class is using {@link #createSessionWithPrivateUser(android
 * .app.Activity,
 * java.util.List)} or {@link #createSessionWithSharedUser(android.app.Activity, java.util.List)} to
 * create a session for a test user. Two modes are supported. In "shared" mode, an attempt is made
 * to find an existing test user that has the required permissions. If no such user is available, a
 * new one is created with the required permissions. In "private" mode, designed for scenarios which
 * require a new user in a known clean state, a new test user will always be created, and it will be
 * automatically deleted when the TestSession is closed. The session obeys the same lifecycle as a
 * regular Session, meaning it must be opened after creation before it can be used to make calls to
 * the Facebook API. <p/> Prior to creating a TestSession, two static methods must be called to
 * initialize the application ID and application Secret to be used for managing test users. These
 * methods are {@link #setTestApplicationId(String)} and {@link #setTestApplicationSecret(String)}.
 * <p/> Note that the shared test user functionality depends on a naming convention for the test
 * users. It is important that any testing of functionality which will mutate the permissions for a
 * test user NOT use a shared test user, or this scheme will break down. If a shared test user seems
 * to be in an invalid state, it can be deleted manually via the Web interface at
...
public class TestSession extends Session {
  ...
  TestSession(Activity activity, List<String> permissions,
      TokenCachingStrategy tokenCachingStrategy, String sessionUniqueUserTag, Mode mode) {
  ...
  /**
   * Constructs a TestSession which creates a test user on open, and destroys the user on close;
   ...
   * @param permissions list of strings containing permissions to request; nil will result in a
   * common set of permissions (email, publish_actions) being requested
   ...
   * creating one
   * if necessary on open (but not deleting it on close, so it can be re-used in later tests).
   * <p/>
   * This method should not be used in application code -- but is useful for creating unit tests
   * that use the Facebook SDK.
   *
  ...
  /**
   * Sets the Facebook Application ID for the application under test. This must be specified prior
   * to creating a TestSession.
   *
  ...
  private static synchronized TestSession createTestSession(Activity activity,
      List<String> permissions, Mode mode, String sessionUniqueUserTag) {
    ...
    return new TestSession(activity, permissions, new TestTokenCachingStrategy(),
        sessionUniqueUserTag, mode);
  }
  ...
  private static synchronized void retrieveTestAccountsForAppIfNeeded() {
    ...
    // The data we need is split across two different FQL tables. We construct two queries,
    // submit them
    // together (the second one refers to the first one), then cross-reference the results.

    // Get the test accounts for this app.
    String testAccountQuery = String.format(
    ...
    // We get back two sets of results. The first is from the test_accounts query,
    // the second from the users query.
    Collection<TestAccount> testAccounts = fqlResults.get(0).getFqlResultSet().castToListOf(
        TestAccount.class);
    Collection<UserAccount> userAccounts = fqlResults.get(1).getFqlResultSet().castToListOf(
        UserAccount.class);
  ...
  private static synchronized void populateTestAccounts(Collection<TestAccount> testAccounts,
    ...
    // single data
    // structure. We have added a Name property to the TestAccount interface,
    // even though we don't really get
  ...
  private void finishAuthWithTestAccount(TestAccount testAccount) {
    ...
    AccessToken accessToken = AccessToken.createFromString(testAccount.getAccessToken(),
        requestedPermissions, AccessTokenSource.TEST_USER);
  ...
  private TestAccount createTestAccountAndFinishAuth() {
    ...
    // If we're in shared mode, we want to rename this user to encode its permissions,
  ...
  private void deleteTestAccount(String testAccountId, String appAccessToken) {
    ...
    if (error != null) {
      Log.w(LOG_TAG, String.format("Could not delete test account %s: %s", testAccountId,
          error.getException().toString()));
    } else if (graphObject.getProperty(Response.NON_JSON_RESPONSE_PROPERTY) == (Boolean) false) {
      Log.w(LOG_TAG, String.format("Could not delete test account %s: unknown reason",
          testAccountId));
    }
  }
  ...
  private String validNameStringFromInteger(long i) {
    ...
    // characters
    //  that might make Facebook reject the name by converting every other repeated character
    // into one
    //  10 higher ('k'-'t').
    char lastChar = 0;
  ...
  private interface TestAccount extends GraphObject {
    ...
    // Note: We don't actually get Name from our FQL query. We fill it in by correlating with
    // UserAccounts.
    String getName();
