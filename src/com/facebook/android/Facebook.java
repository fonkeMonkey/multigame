...
/**
 * THIS CLASS SHOULD BE CONSIDERED DEPRECATED. <p/> All public members of this class are
 * intentionally deprecated. New code should instead use {@link Session} to manage session state,
 * {@link Request} to make API requests, and {@link com.facebook.widget.WebDialog} to make dialog
 * requests. <p/> Adding
 *
...
public class Facebook {
  ...
  @Deprecated
  public static final Uri ATTRIBUTION_ID_CONTENT_URI = Uri.parse(
  ...
  /**
   * Get Attribution ID for app install conversion tracking. <p/> This method is deprecated. See
   * {@link Facebook} and {@link Settings} for more info.
   *
   * @param contentResolver
   * @return Attribution ID that will be used for conversion tracking. It will be null only if the
   * user has not installed or logged in to the Facebook app.
   */
  @Deprecated
   ...
   *
   * @params. <p/> This method is deprecated. See {@link Facebook} and {@link Session} for more
   * info.
   */
  @Deprecated
  ...
  /**
   * Authorize method that grants custom permissions. <p/> See authorize() below for
   *
   * @params. <p/> This method is deprecated. See {@link Facebook} and {@link Session} for more
   * info.
   */
  ...
  /**
   * Full authorize method. <p/> Starts either an Activity or a dialog which prompts the user
   * to log
   * in to Facebook and grant the requested permissions to the given application. <p/> This method
   * will, when possible, use Facebook's single sign-on for Android to obtain an access token.
   * This
   * involves proxying a call through the Facebook for Android stand-alone application, which will
   * handle the authentication flow, and return an OAuth access token for making API calls. <p/>
   ...
   * user credentials are handled by Facebook in an embedded WebView,
   * not by the client application.
   * As such, the dialog makes a network request and renders HTML content rather than a native UI.
   * The access token is retrieved from a redirect to a special URL that the WebView handles. <p/>
   * Note that User credentials could be handled natively using the OAuth 2.0 Username and
   * Password
   * Flow, but this is not supported by this SDK. <p/> See http://developers.facebook
   * .com/docs/authentication/
   * and http://wiki.oauth.net/OAuth-2 for more details. <p/> Note that this method is
   * asynchronous
   * and the callback will be invoked in the original calling thread (not in a background thread).
   * <p/> Also note that requests may be made to the API without calling authorize first, in which
   * case only public information is returned. <p/> IMPORTANT: Note that single sign-on
   * authentication will not function correctly if you do not include a call to the
   * authorizeCallback() method in your onActivityResult() function! Please see below for more
   * information. single sign-on may be disabled by passing FORCE_DIALOG_AUTH as the activityCode
   * parameter in your call to authorize(). <p/> This method is deprecated. See {@link
   * Facebook} and
   * {@link Session} for more info.
   *
   ...
   * "publish_stream", "offline_access", etc. see http://developers.facebook
   * .com/docs/authentication/permissions
   * This parameter should not be null -- if you do not require any permissions, then pass in an
   * empty String array.
   * @param activityCode Single sign-on requires an activity result to be called back to the
   * client
   * application -- if you are waiting on other activities to return data, pass a custom activity
   * code here to avoid collisions. If you would like to force the use of legacy dialog-based
   * authorization, pass FORCE_DIALOG_AUTH for this parameter. Otherwise just omit this parameter
   * and Facebook will use a suitable default. See http://developer.android.com/reference/android/
   ...
   * authorizeCallback() method in your onActivityResult() function! Please see below for more
   * information. single sign-on may be disabled by passing FORCE_DIALOG_AUTH as the activityCode
   * parameter in your call to authorize().
   *
   * @param activity The Android activity in which we want to display the authorization dialog.
   * @param permissions A list of permissions required for this application: e.g. "read_stream",
   * "publish_stream", "offline_access", etc. see http://developers.facebook
   ...
   * and Facebook will use a suitable default. See http://developer.android.com/reference/android/
   * app/Activity.html for more information.
   * @param behavior The {@link SessionLoginBehavior SessionLoginBehavior} that specifies what
    ...
    if (state == SessionState.OPENED) {
      ...
      if (exception instanceof FacebookOperationCanceledException) {
        listener.onCancel();
          ...
          extras.containsKey(Session.WEB_VIEW_ERROR_CODE_KEY) && extras.containsKey(
          Session.WEB_VIEW_FAILING_URL_KEY)) {
        DialogError error = new DialogError(exception.getMessage(), extras.getInt(
            Session.WEB_VIEW_ERROR_CODE_KEY), extras.getString(Session.WEB_VIEW_FAILING_URL_KEY));
  ...
  /**
   * Helper to validate a service intent by resolving and checking the provider's package
   * signature.
   *
   * @param context
   * @param intent
  ...
  /**
   * Query the signature for the application that would be invoked by the given intent and verify
   * that it matches the FB application's signature.
   *
   * @param context
   * @param packageName
  ...
  /**
   * IMPORTANT: If you are using the deprecated authorize() method, this method must be invoked at
   * the top of the calling activity's onActivityResult() function or Facebook authentication will
   * not function properly! <p/> If your calling activity does not currently implement
   * onActivityResult(), you must implement it and include a call to this method if you intend to
   * use the authorize() method in this SDK. <p/> For more information, see
   * http://developer.android.com/reference/android/app/ Activity.html#onActivityResult(int, int,
   * android.content.Intent) <p/> This method is deprecated. See {@link Facebook} and {@link
  ...
  public void authorizeCallback(int requestCode, int resultCode, Intent data) {
    ...
    if (pending != null) {
      if (pending.onActivityResult(this.pendingAuthorizationActivity, requestCode, resultCode,
          data)) {
  ...
  /**
   * Refresh OAuth access token method. Binds to Facebook for Android stand-alone application
   * application to refresh the access token. This method tries to connect to the Facebook App
   * which
   * will handle the authentication flow, and return a new OAuth access token. This method will
   * automatically replace the old token with a new one. Note that this method is asynchronous and
   * the callback will be invoked in the original calling thread (not in a background thread).
   * <p/>
   * This method is deprecated. See {@link Facebook} and {@link Session} for more info.
   *
   * @param context The Android Context that will be used to bind to the Facebook RefreshToken
   * Service
   * @param serviceListener Callback interface for notifying the calling application when the
   * refresh request has completed or failed (can be null). In case of a success a new token can be
  ...
  /**
   * Calls extendAccessToken if shouldExtendAccessToken returns true. <p/> This method is
   * deprecated. See {@link Facebook} and {@link Session} for more info.
   *
   * @return the same value as extendAccessToken if the the token requires refreshing, true
   * otherwise
   */
  @Deprecated
  ...
  /**
   * Check if the access token requires refreshing. <p/> This method is deprecated. See {@link
   * Facebook} and {@link Session} for more info.
   *
   ...
   * browser cookie, and calling auth.expireSession through the API. <p/> Note that this method
   * blocks waiting for a network response, so do not call it in a UI thread. <p/> This method is
   * deprecated. See {@link Facebook} and {@link Session} for more info.
   *
   ...
   * parameter keys must be "method" and its value should be a valid REST server API method. <p/>
   * See http://developers.facebook.com/docs/reference/rest/ <p/> Note that this method blocks
   * waiting for a network response, so do not call it in a UI thread. <p/> Example: <code> Bundle
   * parameters = new Bundle(); parameters.putString("method", "auth.expireSession"); String
   * response = request(parameters); </code> <p/> This method is deprecated. See {@link Facebook}
   * and {@link Request} for more info.
   *
   * @param parameters Key-value pairs of parameters to the request. Refer to the documentation: one
   * of the parameters must be "method".
  ...
  /**
   * Make a request to the Facebook Graph API without any parameters. <p/> See
   * http://developers.facebook.com/docs/api <p/> Note that this method blocks waiting for a network
   * response, so do not call it in a UI thread. <p/> This method is deprecated. See {@link
   * Facebook} and {@link Request} for more info.
   *
   ...
   * (default method). <p/> See http://developers.facebook.com/docs/api <p/> Note that this method
   * blocks waiting for a network response, so do not call it in a UI thread. <p/> This method is
   * deprecated. See {@link Facebook} and {@link Request} for more info.
   *
   * @param graphPath Path to resource in the Facebook graph, e.g., to fetch data about the
   * currently logged authenticated user, provide "me", which will fetch
   * http://graph.facebook.com/me
   * @param parameters key-value string parameters, e.g. the path "search" with parameters "q" :
   ...
   * helper function. <p/> See http://developers.facebook.com/docs/api <p/> Note that this method
   * blocks waiting for a network response, so do not call it in a UI thread. <p/> This method is
   * deprecated. See {@link Facebook} and {@link Request} for more info.
   *
   * @param graphPath Path to resource in the Facebook graph, e.g., to fetch data about the
  ...
  /**
   * Generate a UI dialog for the request action in the given Android context. <p/> Note that this
   * method is asynchronous and the callback will be invoked in the original calling thread
   * (not in
   * a background thread). <p/> This method is deprecated. See {@link
   ...
   * parameters. <p/> Note that this method is asynchronous and the callback will be invoked in the
   * original calling thread (not in a background thread). <p/> This method is deprecated. See
  ...
  private void checkUserSession(String methodName) {
    if (userSetSession != null) {
      throw new UnsupportedOperationException(String.format(
          "Cannot call %s after setSession has been called.", methodName));
    }
  }
  ...
  public final Session getSession() {
    while (true) {
      ...
      Session.OpenRequest openRequest = new Session.OpenRequest(pendingAuthorizationActivity)
  ...
  /**
   * Allows the user to set a Session for the Facebook class to use. If a Session is set here,
   * then
   * one should not use the authorize, logout, or extendAccessToken methods which alter the Session
   * object since that may result in undefined behavior. Using those methods after setting the
  ...
  /**
   * Retrieve the OAuth 2.0 access token for API access: treat with care. Returns null if no session
   * exists.
   *
  ...
  /**
   * Retrieve the current session's expiration time (in milliseconds since Unix epoch), or 0 if the
   * session doesn't expire or doesn't exist.
   *
   ...
   * session doesn't expire. <p/> This method is deprecated. See {@link Facebook} and {@link
   * Session} for more info.
   *
  ...
  /**
   * Retrieve the last time the token was updated (in milliseconds since the Unix epoch), or 0 if
  ...
  /**
   * Restore the token, expiration time, and last update time from cached values. These should be
   * values obtained from getAccessToken(), getAccessExpires, and getLastAccessUpdate()
   * respectively. <p/> This method is deprecated. See {@link Facebook} and {@link Session} for
   * more
   * info.
   *
   ...
   * expire. <p/> This method is deprecated. See {@link Facebook} and {@link Session} for more
   * info.
   *
  ...
  public void setAccessExpiresIn(String expiresInSecsFromNow) {
    ...
    if (expiresInSecsFromNow != null) {
      long expires = expiresInSecsFromNow.equals("0") ? 0 :
          System.currentTimeMillis() + Long.parseLong(expiresInSecsFromNow) * 1000L;
   ...
   * authorize(), unless it has occurred previously or the app does not have install attribution
   * enabled on the application's developer config page. <p/> This method is deprecated. See
   * {@link
   * Facebook} and {@link Settings} for more info.
   *
  ...
  /**
   * Sets whether auto publishing of installs will occur. <p/> This method is deprecated. See
   * {@link
   * Facebook} and {@link Settings} for more info.
   *
   ...
   * repeat
   * calls to prevent multiple installs being published to the graph. <p/> This method is
   * deprecated. See {@link Facebook} and {@link Settings} for more info.
   *
   * @param context the current Android context
   * @return Always false. Earlier versions of the API returned true if it was no longer necessary
   * to call. Apps should ignore this value, but for compatibility we will return false to ensure
   * repeat calls (and the underlying code will prevent duplicate network traffic).
   */
  @Deprecated
  ...
  /**
   * Callback interface for dialog requests. <p/> THIS CLASS SHOULD BE CONSIDERED DEPRECATED. <p/>
   * All public members of this class are intentionally deprecated. New code should instead use
   * {@link com.facebook.widget.WebDialog} <p/> Adding
   *
   * @Deprecated to this class causes warnings in other deprecated classes that reference this one.
   * That is the only reason this entire class is not deprecated.
   * @devDocDeprecated
  ...
  public static interface DialogListener {
    ...
    /**
     * Called when a dialog completes. <p/> Executed by the thread that initiated the dialog.
     *
    ...
    /**
     * Called when a Facebook responds to a dialog with an error. <p/> Executed by the thread
     * that
    ...
    /**
     * Called when a dialog has an error. <p/> Executed by the thread that initiated the dialog.
     */
    public void onError(DialogError e);

    /**
     * Called when a dialog is canceled by the user. <p/> Executed by the thread that
     * initiated the
     * dialog.
     */
  ...
  /**
   * Callback interface for service requests. <p/> THIS CLASS SHOULD BE CONSIDERED DEPRECATED. <p/>
   * All public members of this class are intentionally deprecated. New code should instead use
   * {@link Session} to manage session state. <p/> Adding
   *
   * @Deprecated to this class causes warnings in other deprecated classes that reference this one.
   * That is the only reason this entire class is not deprecated.
   * @devDocDeprecated
    ...
    public void handleMessage(Message msg) {
      ...
      if (token != null) {
        ...
        if (refreshSession != null) {
          // Session.internalRefreshToken expects the original bundle with expires_in in seconds
          // from
          // epoch.
  ...
  private class TokenRefreshServiceConnection implements ServiceConnection {
    ...
    final Messenger messageReceiver = new Messenger(new TokenRefreshConnectionHandler(Facebook.this,
        this));
  ...
  private class SetterTokenCachingStrategy extends TokenCachingStrategy {
    ...
    public void save(Bundle bundle) {
      ...
      lastAccessUpdateMillisecondsAfterEpoch = TokenCachingStrategy.getLastRefreshMilliseconds(
          bundle);
    }
