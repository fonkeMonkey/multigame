...
/**
 * This class represents an error that occurred during a Facebook request. <p/> In general, one
 * would call {@link #getCategory()} to determine the type of error that occurred, and act
 * accordingly. The app can also call {@link #getUserActionMessageId()} in order to get the resource
 * id for a string that can be displayed to the user. For more information on error handling, see <a
...
public final class FacebookRequestError {
  ...
  private FacebookRequestError(int requestStatusCode, int errorCode, int subErrorCode,
      String errorType, String errorMessage, JSONObject requestResultBody, JSONObject requestResult,
      Object batchRequestResult, HttpURLConnection connection, FacebookException exception) {
  ...
  private FacebookRequestError(int requestStatusCode, int errorCode, int subErrorCode,
    ...
    this(requestStatusCode, errorCode, subErrorCode, errorType, errorMessage, requestResultBody,
        requestResult, batchRequestResult, connection, null);
  }

  FacebookRequestError(HttpURLConnection connection, Exception exception) {
    this(INVALID_HTTP_STATUS_CODE, INVALID_ERROR_CODE, INVALID_ERROR_CODE, null, null, null, null,
        null, connection, (exception instanceof FacebookException) ? (FacebookException) exception :
            new FacebookException(exception));
  }

  public FacebookRequestError(int errorCode, String errorType, String errorMessage) {
    this(INVALID_HTTP_STATUS_CODE, errorCode, INVALID_ERROR_CODE, errorType, errorMessage, null,
        null, null, null, null);
  }
      ...
      if (singleResult.has(CODE_KEY)) {
        ...
        if (body != null && body instanceof JSONObject) {
          ...
          if (jsonBody.has(ERROR_KEY)) {
            ...
            JSONObject error = (JSONObject) Utility.getStringPropertyAsJSON(jsonBody, ERROR_KEY,
                null);
            ...
            hasError = true;
          } else if (jsonBody.has(ERROR_CODE_KEY) || jsonBody.has(ERROR_MSG_KEY) || jsonBody.has(
              ERROR_REASON_KEY)) {
          ...
          if (hasError) {
            return new FacebookRequestError(responseCode, errorCode, errorSubCode, errorType,
                errorMessage, jsonBody, singleResult, batchResult, connection);
          }
        }
        ...
        if (!HTTP_RANGE_SUCCESS.contains(responseCode)) {
          return new FacebookRequestError(responseCode, INVALID_ERROR_CODE, INVALID_ERROR_CODE,
              null, null, singleResult.has(BODY_KEY) ? (JSONObject) Utility.getStringPropertyAsJSON(
              singleResult, BODY_KEY, Response.NON_JSON_RESPONSE_PROPERTY) : null, singleResult,
  ...
  /**
   * Returns the resource id for a user-friendly message for the application to present to the
   * user.
   *
   ...
   * } in
   * order to get a resource id for a message to show the user.
   *
  ...
  /**
   * Returns the category in which the error belongs. Applications can use the category to
   * determine
   ...
   * then the result from this method is the same as {@link #getRequestResult()}. In case of a
   * batch
  ...
  public String toString() {
    return new StringBuilder("{HttpStatus: ").append(requestStatusCode).append(", errorCode: ")
        .append(errorCode).append(", errorType: ").append(errorType).append(", errorMessage: ")
        .append(getErrorMessage()).append("}").toString();
  }
  ...
  public enum Category {
    ...
    /**
     * Indicates that the error is authentication related, and that the app should retry the
     * request
     * after some user action.
     */
    AUTHENTICATION_RETRY,
    /**
     * Indicates that the error is authentication related, and that the app should close the
     * session
     * and reopen it.
     */
     ...
     * and is
     * likely newer than the current version of the SDK.
     */
    OTHER,
    /**
     * Indicates that the error is an application error resulting in a bad or malformed request to
     * the server.
     */
    BAD_REQUEST,
    /**
     * Indicates that this is a client-side error. Examples of this can include, but are not limited
     * to, JSON parsing errors or {@link java.io.IOException}s.
     */
    CLIENT
  }
