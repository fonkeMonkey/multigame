...
/**
 * This class helps to create, automatically open (if applicable), save, and restore the Active
 * Session in a way that is similar to Android UI lifecycles. <p> When using this class, clients
 * MUST call all the public methods from the respective methods in either an Activity or Fragment.
 * Failure to call all the methods can result in improperly initialized or uninitialized Sessions.
 * <p> This class should also be used by Activities that will be displaying native dialogs provided
 * by the Facebook application, in order to handle processing of the activity results generated by
 * those dialogs.
 */
public class UiLifecycleHelper {
  ...
  /**
   * To be called from an Activity or Fragment's onActivityResult method, when the results of a
   * FacebookDialog call are expected.
   *
   * @param requestCode the request code
   * @param resultCode the result code
   * @param data the result data
  ...
  /**
   * Register that we are expecting results from a call to the Facebook application (e.g., from a
   * native dialog provided by the Facebook app). Activity results forwarded to onActivityResults
   * will be parsed and handled if they correspond to this call. Only a single pending
   * FacebookDialog call can be tracked at a time; attempting to track another one will cancel the
   * first one.
   *
  ...
  /**
   * Retrieves an instance of AppEventsLogger that can be used for the current Session, if any.
   * Different instances may be returned if the current Session changes,
   * so this value should not be
   * cached for long periods of time -- always call getAppEventsLogger to get the right logger for
   * the current Session. If no Session is currently available, this method will return null. <p/>
   * To ensure delivery of app events across Activity lifecycle events,
   * calling Activities should be
   * sure to call the onStop method.
   *
  ...
  private void cancelPendingAppCall(FacebookDialog.Callback facebookDialogCallback) {
    if (facebookDialogCallback != null) {
      ...
      cancelIntent.putExtra(NativeProtocol.EXTRA_PROTOCOL_CALL_ID, pendingIntent.getStringExtra(
          NativeProtocol.EXTRA_PROTOCOL_CALL_ID));
      cancelIntent.putExtra(NativeProtocol.EXTRA_PROTOCOL_ACTION, pendingIntent.getStringExtra(
          NativeProtocol.EXTRA_PROTOCOL_ACTION));
      cancelIntent.putExtra(NativeProtocol.EXTRA_PROTOCOL_VERSION, pendingIntent.getIntExtra(
