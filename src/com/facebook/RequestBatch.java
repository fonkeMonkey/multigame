...
/**
 * RequestBatch contains a list of Request objects that can be sent to Facebook
...
public class RequestBatch extends AbstractList<Request> {
    ...
    /**
     * Gets the timeout to wait for responses from the server before a timeout
     * error occurs.
     *
     * @return the timeout, in milliseconds; 0 (the default) means do not
     * timeout
     */
    ...
    /**
     * Sets the timeout to wait for responses from the server before a timeout
     * error occurs.
     *
    ...
    /**
     * Adds a batch-level callback which will be called when the entire batch
    ...
    /**
     * Executes this batch asynchronously. This function will return
     * immediately, and the batch will be processed on a separate thread. In
     * order to process results of a request, or determine whether a request
     * succeeded or failed, a callback must be specified (see
    ...
    /**
     * Specifies the interface that consumers of the RequestBatch class can
     * implement in order to be notified when the entire batch completes
     * execution. It will be called after all per-Request callbacks are called.
     */
