...
/*
 * Provides an interface for presenting dialogs provided by the Facebook application for Android.
  * This class
 * provides builders that present a strongly-typed interface to generate properly-formed Intents
 * for launching
...
public class FacebookDialog {
  ...
  private static final int MIN_NATIVE_SHARE_PROTOCOL_VERSION =
      NativeProtocol.PROTOCOL_VERSION_20130618;
  ...
  /**
   * Parses the results of a dialog activity and calls the appropriate method on the provided
   * Callback.
   *
   * @param context the Context that is handling the activity result
   * @param appCall an PendingCall containing the call ID and original Intent used to launch the
   * dialog
   ...
   */
  public static boolean handleActivityResult(Context context, PendingCall appCall, int requestCode,
      Intent data, Callback callback) {
  ...
  /**
   * Determines whether the version of the Facebook application installed on the user's device is
   * recent enough to support specific features of the native Share dialog, which in turn may be
   * used to determine which UI, etc., to present to the user.
   *
   * @param context the calling Context
   * @param features zero or more features to check for; {@link
   * ShareDialogFeature#SHARE_DIALOG} is
   * implicitly checked if not explicitly specified
   * @return true if all of the specified features are supported by the currently installed
   * version
   * of the Facebook application; false if any of the features are not supported
   */
  ...
  /**
   * Determines whether the version of the Facebook application installed on the user's device is
   * recent enough to support specific features of the native Open Graph action dialog, which in
   ...
   * @return true if all of the specified features are supported by the currently installed version
   * of the Facebook application; false if any of the features are not supported
   */
  public static boolean canPresentOpenGraphActionDialog(Context context,
      OpenGraphActionDialogFeature... features) {
    return handleCanPresent(context, EnumSet.of(OpenGraphActionDialogFeature.OG_ACTION_DIALOG,
        features));
  }
  ...
  /**
   * Launches an activity in the Facebook application to present the desired dialog. This method
   * returns a PendingCall that contains a unique ID associated with this call to the Facebook
   * application. In general, a calling Activity should use UiLifecycleHelper to handle incoming
   * activity results, in order to ensure proper processing of the results from this dialog.
   *
   * @return a PendingCall containing the unique call ID corresponding to this call to the Facebook
   * application
   */
  ...
  /**
   * Defines a set of features that may be supported by the native Share dialog exposed by the
   * Facebook application. As additional features are added, these flags may be passed to null
   * {@link FacebookDialog#canPresentShareDialog(android.content.Context,
   * com.facebook.widget.FacebookDialog.ShareDialogFeature...)} to determine whether the
   * version of
   * the Facebook application installed on the user's device is recent enough to support specific
   * features, which in turn may be used to determine which UI, etc., to present to the user.
   */
  ...
  /**
   * Defines a set of features that may be supported by the native Open Graph action dialog
   * exposed
   * by the Facebook application. As additional features are added, these flags may be passed to
   * null     {@link FacebookDialog#canPresentOpenGraphActionDialog(android.content.Context,
   * com.facebook.widget.FacebookDialog.OpenGraphActionDialogFeature...)} to determine whether the
   * version of the Facebook application installed on the user's device is recent enough to support
   *
   * specific features, which in turn may be used to determine which UI, etc., to present to the
   * user.
   */
  ...
  /**
   * Defines a callback interface that will be called when the user completes interacting with a
   * Facebook dialog, or if an error occurs.
   */
  public interface Callback {
     ...
     *
     * @param pendingCall a PendingCall containing the call ID and original Intent used to launch
     * the dialog
     * @param data a Bundle containing the results of the dialog,
     * whose contents will vary depending
     * on the type of dialog being displayed.
     */
  ...
  private abstract static class Builder<CONCRETE extends Builder<?>> {
    ...
    /**
     * Sets the request code that will be passed to handleActivityResult when this activity
     * completes; the default is NativeProtocol.DIALOG_REQUEST_CODE.
     *
    ...
    public CONCRETE setRequestCode(int requestCode) {
      ...
      @SuppressWarnings("unchecked") CONCRETE result = (CONCRETE) this;
      return result;
    }

    /**
     * Sets the name of the application to be displayed in the dialog. If provided, this optimizes
     * the user experience as a preview of a shared item, etc., can be displayed sooner.
     *
    ...
    /**
     * Sets the fragment that should launch the dialog. This allows the dialog to be launched from a
     * Fragment, and will allow the fragment to receive the {@link Fragment#onActivityResult(int,
     * int, android.content.Intent) onActivityResult} call rather than the Activity.
     *
    ...
    /**
     * Constructs a FacebookDialog with an Intent that is correctly populated to present the dialog
    ...
    /**
     * Determines whether the native dialog can be presented (i.e., whether the required version of
     * the Facebook application is installed on the device, and whether the installed version
     * supports all of the parameters specified for the dialog).
     *
  ...
  /**
   * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
   * native Share dialog. This builder will throw an exception if the Facebook application is not
   * installed, so it should only be used if {@link FacebookDialog.checkCanPresentShareDialog()}
  ...
  public static class ShareDialogBuilder extends Builder<ShareDialogBuilder> {
     ...
     * Constructor.
     *
     * @param activity the Activity which is presenting the native Share dialog; must not be
     * null
     */
    ...
    /**
     * Sets whether errors encountered during previewing the shared item should be considered fatal
     * and cause the dialog to return an error
     *
    ...
    Intent handleBuild(Bundle extras) {
      ...
      Intent intent = NativeProtocol.createPlatformActivityIntent(activity,
          NativeProtocol.ACTION_FEED_DIALOG, protocolVersion, extras);
      return intent;
    }
  }
  ...
  /**
   * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
   * native Open Graph action publish dialog. This builder allows the caller to specify binary
   * images for both the action and any Open Graph objects to be created prior to publishing the
   * action. This builder will throw an exception if the Facebook application is not installed, so
   * it should only be used if {@link FacebookDialog.checkCanPresentOpenGraphDialog();} indicates
  ...
  public static class OpenGraphActionDialogBuilder extends Builder<OpenGraphActionDialogBuilder> {
     ...
     * Constructor.
     *
     * @param activity the Activity which is presenting the native Open Graph action publish
     * dialog;
     * must not be null
     * @param action the Open Graph action to be published, which must contain a reference to at
     * least one Open Graph object with the property name specified by setPreviewPropertyName; the
     * action must have had its type specified via the {@link OpenGraphAction#setType(String)}
     * method
     * @param actionType the type of the Open Graph action to be published, which should be the
     * namespace-qualified name of the action type (e.g., "myappnamespace:myactiontype"); this will
     * override the type of the action passed in.
     ...
     */
    @Deprecated
    public OpenGraphActionDialogBuilder(Activity activity, OpenGraphAction action,
        String actionType, String previewPropertyName) {
      super(activity);
      ...
      if (action.getProperty(previewPropertyName) == null) {
        throw new IllegalArgumentException(
            "A property named \"" + previewPropertyName + "\" was not found on the " +
                "action.  The name of " +
      ...
      if (!Utility.isNullOrEmpty(typeOnAction) && !typeOnAction.equals(actionType)) {
        throw new IllegalArgumentException("'actionType' must match the type of 'action' " +
            "if it is specified. " +
            "Consider using OpenGraphActionDialogBuilder(Activity activity, " +
            "OpenGraphAction action, " + "String previewPropertyName) instead.");
      }
      this.action = action;
    ...
    /**
     * Sets whether errors encountered during previewing the shared item should be considered
     * fatal
     * and cause the dialog to return an error
     *
     * @param dataErrorsFatal true if data errors should be fatal; false if not
    ...
    /**
     * <p>Specifies a list of images for the Open Graph action that should be uploaded prior to
     * publishing the action. The action must already have been set prior to calling this
     * method.
     * This method will generate unique names for the image attachments and update the action to
     * refer to these attachments. Note that calling setAction again after calling this
     * method will
     * not clear the image attachments already set, but the new action will have no reference
     * to the
     * existing attachments. The images will not be marked as being user-generated.</p> <p/>
     * <p>In
     * order for the images to be provided to the Facebook application as part of the app
     * call, the
     * NativeAppCallContentProvider must be specified correctly in the application's
    ...
    /**
     * <p>Specifies a list of images for the Open Graph action that should be uploaded prior to
     * publishing the action. The action must already have been set prior to calling this method.
     * This method will generate unique names for the image attachments and update the action to
     * refer to these attachments. Note that calling setAction again after calling this method will
     * not clear the image attachments already set, but the new action will have no reference to the
     * existing attachments. The images may be marked as being user-generated -- refer to <a
     * href="https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories
     * /">this
     * article</a> for more information.</p> <p/> <p>In order for the images to be provided
     * to the
     * Facebook application as part of the app call, the NativeAppCallContentProvider must be
     * specified correctly in the application's AndroidManifest.xml.</p>
     *
     * @param bitmaps a list of Bitmaps to be uploaded and attached to the Open Graph action
     * @param isUserGenerated if true, specifies that the user_generated flag should be set for
     * these images
     * @return this instance of the builder
     */
    public OpenGraphActionDialogBuilder setImageAttachmentsForAction(List<Bitmap> bitmaps,
     ...
     * publishing the action. The action must already have been set prior to calling this
     * method.
     * The images will not be marked as being user-generated. This method will generate
     * unique names
     * for the image attachments and update the action to refer to these attachments. Note that
     * calling setAction again after calling this method will not clear the image attachments
     * already set, but the new action will have no reference to the existing attachments
     * .</p> <p/>
     * <p>In order for the images to be provided to the Facebook application as part of the app
     * call, the NativeAppCallContentProvider must be specified correctly in the application's
     * AndroidManifest.xml.</p>
     *
     * @param bitmapFiles a list of Files containing bitmaps to be uploaded and attached to
     * the Open
     * Graph action
     * @return this instance of the builder
     */
     ...
     * <p>Specifies a list of images for the Open Graph action that should be uploaded prior to
     * publishing the action. The action must already have been set prior to calling this method.
     * The images may be marked as being user-generated -- refer to <a
     * href="https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories/">this
     * article</a> for more information. This method will generate unique names for the image
     * attachments and update the action to refer to these attachments. Note that calling setAction
     * again after calling this method will not clear the image attachments already set, but the new
     * action will have no reference to the existing attachments.</p> <p/> <p>In order for the
     * images to be provided to the Facebook application as part of the app call, the
     * NativeAppCallContentProvider must be specified correctly in the application's
     * AndroidManifest.xml.</p>
     *
     * @param bitmapFiles a list of Files containing bitmaps to be uploaded and attached to the Open
     * Graph action
     * @param isUserGenerated if true, specifies that the user_generated flag should be set for
     * these images
     * @return this instance of the builder
    ...
    /**
     * <p>Specifies a list of images for an Open Graph object referenced by the action that should
     * be uploaded prior to publishing the action. The images will not be marked as
     * user-generated.
     * The action must already have been set prior to calling this method,
     * and the action must have
     * a GraphObject-valued property with the specified property name. This method will generate
     * unique names for the image attachments and update the graph object to refer to these
     * attachments. Note that calling setObject again after calling this method,
     * or modifying the
     * value of the specified property, will not clear the image attachments already set,
     * but the
     * new action (or objects) will have no reference to the existing attachments.</p> <p/>
     * <p>In
     * order for the images to be provided to the Facebook application as part of the app
     * call, the
     * NativeAppCallContentProvider must be specified correctly in the application's
     * AndroidManifest.xml.</p>
     *
     * @param objectProperty the name of a property on the action that corresponds to an Open
     * Graph
     * object; the object must be marked as a new object to be created (i.e., {@link
     * com.facebook.model.OpenGraphObject#getCreateObject()} must return true) or an
     * exception will
     * be thrown
     * @param bitmapFiles a list of Files containing bitmaps to be uploaded and attached to
     * the Open
     ...
     * @return this instance of the builder
     */
    public OpenGraphActionDialogBuilder setImageAttachmentsForObject(String objectProperty,
        List<Bitmap> bitmaps) {
      return setImageAttachmentsForObject(objectProperty, bitmaps, false);
    }

    /**
     * <p>Specifies a list of images for an Open Graph object referenced by the action that
     * should
     * be uploaded prior to publishing the action. The images may be marked as being
     * user-generated
     * -- refer to <a href="https://developers.facebook
     * .com/docs/opengraph/howtos/adding-photos-to-stories/">this
     * article</a> for more information. The action must already have been set prior to calling this
     * method, and the action must have a GraphObject-valued property with the specified property
     * name. This method will generate unique names for the image attachments and update the graph
     * object to refer to these attachments. Note that calling setObject again after calling this
     * method, or modifying the value of the specified property, will not clear the image
     * attachments already set, but the new action (or objects) will have no reference to the
     * existing attachments.</p> <p/> <p>In order for the images to be provided to the Facebook
     * application as part of the app call, the NativeAppCallContentProvider must be specified
     * correctly in the application's AndroidManifest.xml.</p>
     *
     * @param objectProperty the name of a property on the action that corresponds to an Open
     * Graph
     * object; the object must be marked as a new object to be created (i.e., {@link
    ...
    /**
     * <p>Specifies a list of images for an Open Graph object referenced by the action that should
     * be uploaded prior to publishing the action. The images may be marked as being user-generated
     * -- refer to <a href="https://developers.facebook
     * .com/docs/opengraph/howtos/adding-photos-to-stories/">this
     * article</a> for more information. The action must already have been set prior to calling this
     * method, and the action must have a GraphObject-valued property with the specified property
     ...
     * correctly in the application's AndroidManifest.xml.</p>
     *
     * @param objectProperty the name of a property on the action that corresponds to an Open Graph
     * object; the object must be marked as a new object to be created (i.e., {@link
     * com.facebook.model.OpenGraphObject#getCreateObject()} must return true) or an exception will
     * be thrown
     * @param bitmaps a list of Bitmaps to be uploaded and attached to the Open Graph object
     * @param isUserGenerated if true, specifies that the user_generated flag should be set for
     * these images
    ...
    void updateObjectAttachmentUrls(String objectProperty, List<String> attachmentUrls,
        boolean isUserGenerated) {
    ...
    private List<String> addImageAttachments(List<Bitmap> bitmaps) {
      ...
      for (Bitmap bitmap : bitmaps) {
        ...
        String url = NativeAppCallContentProvider.getAttachmentUrl(applicationId,
            appCall.getCallId(), attachmentName);
    ...
    @Override
    boolean handleCanPresent() {
      return canPresentOpenGraphActionDialog(activity,
          OpenGraphActionDialogFeature.OG_ACTION_DIALOG);
    }
      ...
      return new OnPresentCallback() {
        ...
        public void onPresent(Context context) throws Exception {
          ...
          if (imageAttachments != null && imageAttachments.size() > 0) {
            getAttachmentStore().addAttachmentsForCall(context, appCall.getCallId(),
                imageAttachments);
          }
    ...
    private JSONObject flattenChildrenOfGraphObject(JSONObject graphObject) {
      try {
        ...
        @SuppressWarnings("unchecked") Iterator<String> keys = graphObject.keys();
        while (keys.hasNext()) {
  ...
  /**
   * Encapsulates information about a call being made to the Facebook application for Android. A
   * unique String call ID is used to track calls through their lifecycle.
   */
