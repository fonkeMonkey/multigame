...
public class ProfilePictureView extends FrameLayout {
   ...
   * ProfilePictureView
   * will default to NORMAL X NORMAL, if the layout params set on this instance do not have a
   * fixed
   ...
   * cropped or un-cropped version is chosen. Used in calls to setPresetSize() and
   * getPresetSize().
   ...
   *
   * @param showCroppedVersion True to select the cropped version False to select the standard
   * version
   */
  ...
  /**
   * Sets an OnErrorListener for this instance of ProfilePictureView to call into when certain
   * exceptions occur.
   *
   ...
   * specifically. <p/> The approach is to default to a NORMAL sized amount of space in the case
   * that a preset size is not specified. This logic is applied to both width and height
   */
  @Override
  ...
  /**
   * In addition to calling super.Layout(), we also attempt to get a new image that is properly size
   * for the layout dimensions
   */
  @Override
  ...
  private void initialize(Context context) {
    ...
    LayoutParams imageLayout = new LayoutParams(LayoutParams.MATCH_PARENT,
  ...
  private void setBlankProfilePicture() {
    if (customizedDefaultProfilePicture == null) {
      int blankImageResource = isCropped() ? R.drawable.com_facebook_profile_picture_blank_square :
      ...
      Bitmap scaledBitmap = Bitmap.createScaledBitmap(customizedDefaultProfilePicture, queryWidth,
          queryHeight, false);
  ...
  private void sendImageRequest(boolean allowCachedResponse) {
    try {
      ImageRequest.Builder requestBuilder = new ImageRequest.Builder(getContext(),
