...
/**
 * A Log In/Log Out button that maintains session state and logs in/out for the app. <p/> This
 * control will create and use the active session upon construction if it has the available data (if
 * the app ID is specified in the manifest). It will also open the active session if it does not
 * require user interaction (i.e. if the session is in the {@link com.facebook
 * .SessionState#CREATED_TOKEN_LOADED}
 * state. Developers can override the use of the active session by calling the {@link
 * #setSession(com.facebook.Session)} method.
 */
public class LoginButton extends Button {
  ...
  public LoginButton(Context context, AttributeSet attrs) {
    ...
    if (attrs.getStyleAttribute() == 0) {
      ...
      this.setTextSize(TypedValue.COMPLEX_UNIT_PX, getResources().getDimension(
      ...
      if (isInEditMode()) {
        ...
        this.setPadding(getResources().getDimensionPixelSize(
            R.dimen.com_facebook_loginview_padding_left), getResources().getDimensionPixelSize(
   ...
   * permissions. If any publish permissions are included, the login attempt by the user will
   * fail.
   * The LoginButton can only be associated with either read permissions or publish
   * permissions, but
   ...
   * between. <p/> This method is only meaningful if called before the session is open. If this is
   * called after the session is opened, and the list of permissions passed in is not a subset of
   * the permissions granted during the authorization, it will log an error. <p/> Since the
   * session
   * can be automatically opened when the LoginButton is constructed,
   * it's important to always pass
   * in a consistent set of permissions to this method, or manage the setting of permissions
   * outside
   ...
   * is called in between. <p/> This method is only meaningful if called before the session is
   * open.
   * If this is called after the session is opened, and the list of permissions passed in is not a
   * subset of the permissions granted during the authorization, it will log an error. <p/> Since
   * the session can be automatically opened when the LoginButton is constructed,
   * it's important to
   * always pass in a consistent set of permissions to this method, or manage the setting of
   * permissions outside of the LoginButton class altogether (by managing the session explicitly).
   *
   * @param permissions the read permissions to use
   ...
   *
   * @return loginBehavior The {@link SessionLoginBehavior SessionLoginBehavior} that specifies
   * what
   ...
   * onActivityResult occurs as part of Facebook authorization flow,
   * this value is the activityCode
   ...
   * used instead. <p/> If the passed in session is currently opened, this method will also attempt
   * to load some user information for display (if needed).
   *
  ...
  /**
   * Sets the fragment that contains this control. This allows the LoginButton to be embedded inside
   * a Fragment, and will allow the fragment to receive the {@link Fragment#onActivityResult(int,
  ...
  private void setButtonText() {
    if (sessionTracker != null && sessionTracker.getOpenSession() != null) {
      setText((logoutText != null) ? logoutText : getResources().getString(
  ...
  /**
   * Specifies a callback interface that will be called when the button's notion of the current user
   * changes (if the fetch_user_info attribute is true for this control).
   */
        ...
        if (confirmLogout) {
          ...
          if (user != null && user.getName() != null) {
            message = String.format(getResources().getString(
                R.string.com_facebook_loginview_logged_in_as), user.getName());
          } else {
            message = getResources().getString(
                R.string.com_facebook_loginview_logged_in_using_facebook);
          }
          AlertDialog.Builder builder = new AlertDialog.Builder(context);
          builder.setMessage(message).setCancelable(true).setPositiveButton(logout,
