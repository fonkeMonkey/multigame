...
/**
 * <p>Basic implementation of a Fragment that uses a Session to perform Single Sign On (SSO). This
 * class is package private, and is not intended to be consumed by external applications.</p> <p/>
 * <p>The method {@link android.support.v4.app.Fragment#onActivityResult} is used to manage the
 * session information, so if you override it in a subclass, be sure to call {@code
 * super.onActivityResult}.</p> <p/> <p>The methods in this class are not thread-safe.</p>
 */
class FacebookFragment extends Fragment {
  ...
  /**
   * Called when the activity that was launched exits. This method manages session information
   * when
   * a session is opened. If this method is overridden in subclasses, be sure to call {@code
   * super.onActivityResult(...)} first.
   */
  @Override
  ...
  /**
   * Called when the session state changes. Override this method to take action on session state
   * changes.
   *
  ...
  /**
   * Gets the access token associated with the current session or null if no session has been
   * created.
   *
  ...
  /**
   * Gets the date at which the current session will expire or null if no session has been
   * created.
   *
  ...
  /**
   * Opens a new session with read permissions. If either applicationID or permissions is null,
   * this
   ...
   * this method will
   * default to using the value from the associated meta-data value. The permissions list cannot be
   * null.
   *
   * @param applicationId the applicationID, can be null
   ...
   * Opens a new session with publish permissions. If either applicationID is null,
   * this method will
   * default to using the value from the associated meta-data value. The permissions list
   * cannot be
   * null.
   *
   * @param applicationId the applicationID, can be null
  ...
  private void openSession(String applicationId, List<String> permissions,
    ...
    if (sessionTracker != null) {
      ...
      if (currentSession == null || currentSession.getState().isClosed()) {
        Session session = new Session.Builder(getActivity()).setApplicationId(applicationId)
            .build();
