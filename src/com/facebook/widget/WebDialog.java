...
public class WebDialog extends Dialog {
  ...
  /**
   * Constructor which can be used to display a dialog with an already-constructed URL and a
   * custom
   * theme.
   *
   * @param context the context to use to display the dialog
  ...
  private Pair<Integer, Integer> getMargins() {
    ...
    if (scaledWidth <= NO_BUFFER_SCREEN_WIDTH) {
      ...
      scaleFactor = MIN_SCALE_FACTOR + ((double) (MAX_BUFFER_SCREEN_WIDTH - scaledWidth)) /
          ((double) (MAX_BUFFER_SCREEN_WIDTH - NO_BUFFER_SCREEN_WIDTH)) * (1.0 - MIN_SCALE_FACTOR);
    }
  ...
  public static class Builder extends BuilderBase<Builder> {
     ...
     * @param session the Session representing an authenticating user to use for showing the
     * dialog;
     * must not be null, and must be opened.
  ...
  public static class FeedDialogBuilder extends BuilderBase<FeedDialogBuilder> {
     ...
     *
     * @param context the Context within which the dialog will be shown.
     * @param parameters a Bundle containing parameters to pass as part of the dialog URL. No
     * validation is done on these parameters; it is the caller's responsibility to ensure
     * they are
     * valid. For more information, see <a href="https://developers.facebook
     * .com/docs/reference/dialogs/feed/">
    ...
    /**
     * Sets the ID of the profile that is posting to Facebook. If none is specified, the default is
     * "me". This profile must be either the authenticated user or a Page that the user is an
     * administrator of.
     *
    ...
    /**
     * Sets the ID of the profile that the story will be published to. If not specified, it will
     * default to the same profile that the story is being published from.
     *
    ...
    /**
     * Sets the URL of a media file attached to this post. If this is set, any picture set via
     * setPicture will be ignored.
     *
  ...
  public static class RequestsDialogBuilder extends BuilderBase<RequestsDialogBuilder> {
     ...
     * they are
     * valid. For more information, see <a href="https://developers.facebook
     * .com/docs/reference/dialogs/requests/">
  ...
  private class DialogWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
      ...
      if (url.startsWith(WebDialog.REDIRECT_URI)) {
        ...
        } else if (error != null && (error.equals("access_denied") || error.equals(
            "OAuthAccessDeniedException"))) {
          sendCancelToListener();
        } else {
          FacebookRequestError requestError = new FacebookRequestError(errorCode, error,
              errorMessage);
    ...
    public void onPageFinished(WebView view, String url) {
      ...
      if (!isDetached) {
        spinner.dismiss();
            ...
            /*
             * Once web view is fully loaded, set the contentFrameLayout background to be
             * transparent
