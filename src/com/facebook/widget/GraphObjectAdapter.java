...
class GraphObjectAdapter<T extends GraphObject> extends BaseAdapter implements SectionIndexer {
  ...
  private static int compareGraphObjects(GraphObject a, GraphObject b,
      Collection<String> sortFields, Collator collator) {
  ...
  public void prioritizeViewRange(int firstVisibleItem, int lastVisibleItem, int prefetchBuffer) {
    ...
    for (T graphObject : graphObjectsToPrefetchPicturesFor) {
      ...
      // This URL already have been requested for pre-fetching, but we want to act in an LRU
      // manner, so move
  ...
  private View getActivityCircleView(View convertView, ViewGroup parent) {
    ...
    ProgressBar activityCircle = (ProgressBar) result.findViewById(
        R.id.com_facebook_picker_row_activity_circle);
  ...
  protected View createGraphObjectView(T graphObject) {
    ...
    ViewStub profilePicStub = (ViewStub) result.findViewById(
        R.id.com_facebook_picker_profile_pic_stub);
    if (!getShowPicture()) {
  ...
  private boolean shouldShowActivityCircleCell() {
    ...
    return (cursor != null) && cursor.areMoreObjectsAvailable() && (dataNeededListener != null) &&
        !isEmpty();
  }
  ...
  public Object getItem(int position) {
    SectionAndItem<T> sectionAndItem = getSectionAndItem(position);
    return (sectionAndItem.getType() == SectionAndItem.Type.GRAPH_OBJECT) ?
        sectionAndItem.graphObject : null;
  }
