...
public class PlacePickerFragment extends PickerFragment<GraphPlace> {
  ...
  /**
   * The key for an int parameter in the fragment's Intent bundle to indicate the radius in meters
   * around the center point to search. The default is 1000 meters.
   */
  public static final String RADIUS_IN_METERS_BUNDLE_KEY =
      "com.facebook.widget.PlacePickerFragment.RadiusInMeters";
  /**
   * The key for an int parameter in the fragment's Intent bundle to indicate what how many
   * results
   * to return at a time. The default is 100 results.
   */
  public static final String RESULTS_LIMIT_BUNDLE_KEY =
      "com.facebook.widget.PlacePickerFragment.ResultsLimit";
  /**
   * The key for a String parameter in the fragment's Intent bundle to indicate what search text
   * should be sent to the service. The default is to have no search text.
   */
  public static final String SEARCH_TEXT_BUNDLE_KEY =
      "com.facebook.widget.PlacePickerFragment" + ".SearchText";
  /**
  ...
  public static final String LOCATION_BUNDLE_KEY =
      "com.facebook.widget.PlacePickerFragment" + ".Location";
  /**
  ...
  public static final String SHOW_SEARCH_BOX_BUNDLE_KEY =
      "com.facebook.widget" + ".PlacePickerFragment.ShowSearchBox";
  /**
  ...
  /**
   * Gets the location to search around. Either the location or the search text (or both) must be
   * specified.
   *
  ...
  /**
   * Sets the location to search around. Either the location or the search text (or both) must be
   * specified.
   *
   ...
   * functionality where the user may be typing or editing text rapidly. It uses a timer to avoid
   * repeated requerying, preferring to wait until the user pauses typing to refresh the data.
   * Note
   * that this method will NOT update the text in the search box, if any, as it is intended to be
   * called as a result of changes to the search box (and is public to enable applications to
   * provide their own search box UI instead of the default one).
   *
   * @param searchText the search text
   * @param forceReloadEventIfSameText if true, will reload even if the search text has not changed;
   * if false, identical search text will not force a reload
   */
  ...
  public void onInflate(Activity activity, AttributeSet attrs, Bundle savedInstanceState) {
    ...
    setRadiusInMeters(a.getInt(R.styleable.com_facebook_place_picker_fragment_radius_in_meters,
        radiusInMeters));
    ...
    showSearchBox = a.getBoolean(R.styleable.com_facebook_place_picker_fragment_show_search_box,
        showSearchBox);
    ...
    PickerFragmentAdapter<GraphPlace> adapter = new PickerFragmentAdapter<GraphPlace>(
      ...
      protected CharSequence getSubTitleOfGraphObject(GraphPlace graphObject) {
        ...
        if (category != null && wereHereCount != null) {
          result = getString(R.string.com_facebook_placepicker_subtitle_format, category,
              wereHereCount);
        } else if (category == null && wereHereCount != null) {
          result = getString(R.string.com_facebook_placepicker_subtitle_were_here_only_format,
              wereHereCount);
        } else if (category != null && wereHereCount == null) {
          result = getString(R.string.com_facebook_placepicker_subtitle_catetory_only_format,
              category);
        }
        return result;
      }
  ...
  private Request createRequest(Location location, int radiusInMeters, int resultsLimit,
      String searchText, Set<String> extraFields, Session session) {
