...
class AuthorizationClient implements Serializable {
  ...
  // Note: to ensure stability of column mappings across the four different event types,
  // we prepend a column
  ...
  // Constants for logging login-related data. Some of these are only used by Session,
  // but grouped here for
  // maintainability.
  ...
  boolean checkInternetPermission() {
    ...
    if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
      ...
      String errorDescription = context.getString(
  ...
  void tryNextHandler() {
    if (currentHandler != null) {
      logAuthorizationMethodComplete(currentHandler.getNameForLogging(),
          EVENT_PARAM_METHOD_RESULT_SKIPPED, null, null, currentHandler.methodLoggingExtras);
    }
  ...
  boolean tryCurrentHandler() {
    ...
    if (tried) {
      ...
      // We didn't try it, so we don't get any other completion notification -- log that we
      // skipped it.
  ...
  void complete(Result outcome) {
    // This might be null if, for some reason, none of the handlers were successfully tried (in
    // which case
  ...
  RequestBatch createReauthValidationBatch(final Result pendingResult) {
    ...
    // a "me" request using the current token, a "me" request using the new token,
    // and a "me/permissions"
    ...
    batch.addCallback(new RequestBatch.Callback() {
      ...
      public void onBatchCompleted(RequestBatch batch) {
        try {
          Result result = null;
          if (fbids.size() == 2 && fbids.get(0) != null && fbids.get(1) != null && fbids.get(0)
              .equals(fbids.get(1))) {
  ...
  private AppEventsLogger getAppEventsLogger() {
    if (appEventsLogger == null ||
        appEventsLogger.getApplicationId() != pendingRequest.getApplicationId()) {
  ...
  private void logAuthorizationMethodComplete(String method, Result result,
      Map<String, String> loggingExtras) {
    logAuthorizationMethodComplete(method, result.code.getLoggingValue(), result.errorMessage,
        result.errorCode, loggingExtras);
  }

  private void logAuthorizationMethodComplete(String method, String result, String errorMessage,
  ...
  static class Result implements Serializable {
    ...
    static Result createErrorResult(AuthorizationRequest request, String errorType,
  ...
  class WebViewAuthHandler extends AuthHandler {
    ...
    boolean tryAuthorize(final AuthorizationRequest request) {
      ...
      WebDialog.Builder builder = new AuthDialogBuilder(
          getStartActivityDelegate().getActivityContext(), applicationId, parameters).setE2E(e2e)
  ...
  class GetTokenAuthHandler extends AuthHandler {
    ...
    void getTokenCompleted(AuthorizationRequest request, Bundle result) {
      ...
      if (result != null) {
        ArrayList<String> currentPermissions = result.getStringArrayList(
            NativeProtocol.EXTRA_PERMISSIONS);
  ...
  class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
    ...
    boolean tryAuthorize(AuthorizationRequest request) {
      ...
      Intent intent = NativeProtocol.createLoginDialog20121101Intent(context,
          request.getApplicationId(), new ArrayList<String>(request.getPermissions()),
      ...
      addLoggingExtra(EVENT_EXTRAS_PROTOCOL_VERSION, intent.getIntExtra(
          NativeProtocol.EXTRA_PROTOCOL_VERSION, 0));
      addLoggingExtra(EVENT_EXTRAS_PERMISSIONS, TextUtils.join(",", intent.getStringArrayListExtra(
          NativeProtocol.EXTRA_PERMISSIONS)));
      addLoggingExtra(EVENT_EXTRAS_WRITE_PRIVACY, intent.getStringExtra(
          NativeProtocol.EXTRA_WRITE_PRIVACY));
    ...
    private Result handleResultOk(Intent data) {
      ...
      if (errorType == null) {
        return Result.createTokenResult(pendingRequest, AccessToken.createFromNativeLogin(extras,
    ...
    private Result createCancelOrErrorResult(AuthorizationRequest request, Intent data) {
      ...
      if (NativeProtocol.ERROR_USER_CANCELED.equals(errorType) ||
        ...
        return Result.createCancelResult(request, data.getStringExtra(
        ...
        return Result.createErrorResult(request, errorType, data.getStringExtra(
            NativeProtocol.STATUS_ERROR_DESCRIPTION), errorCode);
      }
    }
