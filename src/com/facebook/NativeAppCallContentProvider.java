 ...
 * attach and clean up the attachments. <p/> <p>Note that this ContentProvider is only necessary if
 * an application wishes to attach images, etc., that are stored in memory and do not have another
 * way to be referenced by a content URI. For images obtained from, e.g., the Camera or Gallery,
 * that already have a content URI associated with them, use of this class is not necessary.</p>
 * <p/> <p>If an application wishes to attach images that are stored in-memory within the
 * application, this content provider must be listed in the application's AndroidManifest.xml, and
 * it should be named according to the pattern <code>"com.facebook.app
 * .NativeAppCallContentProvider{FACEBOOK_APP_ID}"</code>.
 * See the {@link NativeAppCallContentProvider#getAttachmentUrl(String) getContentProviderName}
 * method.</p>
 */
public class NativeAppCallContentProvider extends ContentProvider {
  ...
  private static final String ATTACHMENT_URL_BASE =
      "content://com.facebook.app.NativeAppCallContentProvider";
