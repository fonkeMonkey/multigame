...
/**
 * <p> The AppEventsLogger class allows the developer to log various types of events back to
 * Facebook. In order to log events, the app must create an instance of this class via a {@link
 * #newLogger newLogger} method, and then call the various "log" methods off of that. </p> <p> This
 * client-side event logging is then available through Facebook App Insights and for use with
 * Facebook Ads conversion tracking and optimization. </p> <p> The AppEventsLogger class has a few
 * related roles: <ul> <li> Logging predefined and application-defined events to Facebook App
 * Insights with a numeric value to sum across a large number of events, and an optional set of
 * key/value parameters that define "segments" for this event (e.g., 'purchaserStatus' : 'frequent',
 * or 'gamerLevel' : 'intermediate'). These events may also be used for ads conversion tracking,
 * optimization, and other ads related targeting in the future. </li> <li> Methods that control the
 * way in which events are flushed out to the Facebook servers. </li> </ul> Here are some important
 * characteristics of the logging mechanism provided by AppEventsLogger: <ul> <li> Events are not
 * sent immediately when logged. They're cached and flushed out to the Facebook servers in a number
 * of situations: <ul> <li>when an event count threshold is passed (currently 100 logged
 * events).</li> <li>when a time threshold is passed (currently 60 seconds).</li> <li>when an app
 * has gone to background and is then brought back to the foreground.</li> </ul> <li> Events will be
 * accumulated when the app is in a disconnected state, and sent when the connection is restored and
 * one of the above 'flush' conditions are met. </li> <li> The AppEventsLogger class is intended to
 * be used from the thread it was created on. Multiple AppEventsLoggers may be created on other
 * threads if desired. </li> <li> The developer can call the setFlushBehavior method to force the
 * flushing of events to only occur on an explicit call to the `flush` method. </li> <li> The
 * developer can turn on console debug output for event logging and flushing to the server
 * Settings.addLoggingBehavior(LoggingBehavior.APP_EVENTS); </li> </ul> Some things to note when
 * logging events: <ul> <li> There is a limit on the number of unique event names an app can use, on
 * the order of 300. </li> <li> There is a limit to the number of unique parameter names in the
 * provided parameters that can be used per event, on the order of 10. This is not just for an
 * individual call, but for all invocations for that eventName. </li> <li> Event names and parameter
 * names (the keys in the NSDictionary) must be between 2 and 40 characters, and must consist of
 * alphanumeric characters, _, -, or spaces. </li> <li> The length of each parameter value can be no
 * more than on the order of 100 characters. </li> </ul>
 *
 * @beta
 */
public class AppEventsLogger {
  ...
  /**
   * The action used to indicate that a flush of app events has occurred. This should be used
   * as an
  ...
  private static Map<String, EventSuppression> mapEventNameToSuppress =
      new HashMap<String, EventSuppression>() {
        {
  ...
  /**
   * Sets whether events sent to Facebook should be restricted from being used for purposes than
   * analytics and conversions, such as remarketing for this user. Defaults to false. This
   * value is
   * stored on the device and persists across app launches.
   *
   * @param context Used to read the value.
   * @beta
   */
  public static boolean getLimitEventUsage(Context context) {
    SharedPreferences preferences = context.getSharedPreferences(APP_EVENT_PREFERENCES,
   ...
   * analytics and conversions, such as remarketing for this user. Defaults to false. This
   * value is
   * stored on the device and persists across app launches. Changes to this setting will apply to
   * any events currently queued to be flushed, as well as any subsequent events.
   *
   ...
   * long-running Activity of your app. <p/> Use this method if your application ID is stored in
   * application metadata, otherwise see {@link AppEventsLogger#activateApp(android.content
   * .Context,
   * String)}.
   *
  ...
  public static void activateApp(Context context, String applicationId) {
    ...
    // activateApp supercedes publishInstall in the public API, so we need to explicitly invoke
    // it, since the server
  ...
  /**
   * Build an AppEventsLogger instance to log events through. The Facebook app that these
   * events are
   * targeted at comes from this application's metadata. The application ID used to log events
   * will
   * be determined from the app ID specified in the package metadata.
   *
   * @param context Used to access the applicationId and the attributionId for non-authenticated
   * users.
   ...
   * will be used if it's open, otherwise the logging will happen against the default app ID
   * specified via the app ID specified in the package metadata.
   * @return AppEventsLogger instance to invoke log* methods on.
   * @beta
   */
   ...
   * @param session Explicitly specified Session to log events against. If null, the activeSession
   * will be used if it's open, otherwise the logging will happen against the specified app ID.
   * @return AppEventsLogger instance to invoke log* methods on.
   * @beta
   */
   ...
   * flush. Note that this call may trigger an I/O operation on the calling thread. Explicit use of
   * this method is not necessary if the consumer is making use of {@link UiLifecycleHelper}, which
   * will take care of making the call in its own onStop() callback.
   *
   * @beta
   */
  ...
  private static boolean shouldSuppressEvent(AppEvent event) {
    ...
    // Update the time if we're not suppressed, OR if we are suppressed but the behavior is to
    // reset even on
    // suppressed events.
    if (!suppressed || suppressionInfo.getBehavior() ==
        SuppressionTimeoutBehavior.RESET_TIMEOUT_WHEN_LOG_ATTEMPTED) {
  ...
  private static FlushStatistics buildAndExecuteRequests(FlushReason reason,
    ...
    for (AccessTokenAppIdPair accessTokenAppId : keysToFlush) {
      ...
      Request request = buildRequestForSession(accessTokenAppId, sessionEventsState,
          limitEventUsage, flushResults);
      if (request != null) {
    ...
    if (requestsToExecute.size() > 0) {
      Logger.log(LoggingBehavior.APP_EVENTS, TAG, "Flushing %d events due to %s.",
          flushResults.numEvents, reason.toString());

      for (Request request : requestsToExecute) {
        // Execute the request synchronously. Callbacks will take care of handling errors and
        // updating
  ...
  private static Request buildRequestForSession(final AccessTokenAppIdPair accessTokenAppId,
    ...
    final Request postRequest = Request.newPostRequest(null, String.format("%s/activities",
  ...
  private static void handleResponse(AccessTokenAppIdPair accessTokenAppId, Request request,
      Response response, SessionEventsState sessionEventsState, FlushStatistics flushState) {
    ...
    if (flushResult == FlushResult.NO_CONNECTIVITY) {
      // We may call this for multiple requests in a batch, which is slightly inefficient since
      // in principle
      // we could call it once for all failed requests, but the impact is likely to be minimal.
      // We don't call this for other server errors, because if an event failed because it was
      // malformed, etc.,
  ...
  private static int accumulatePersistedEvents() {
    ...
    for (AccessTokenAppIdPair accessTokenAppId : persistedEvents.keySet()) {
      SessionEventsState sessionEventsState = getSessionEventsState(applicationContext,
          accessTokenAppId);
   ...
   * constants
   * in {@link AppEventsConstants} when possible. Or create your own if none of the EVENT_NAME_*
   * constants are applicable. Event names should be 40 characters or less, alphanumeric, and can
   * include spaces, underscores or hyphens, but mustn't have a space or hyphen as the first
   * character. Any given app should have no more than ~300 distinct event names.
   * @beta
   */
   ...
   * constants are applicable. Event names should be 40 characters or less, alphanumeric, and can
   * include spaces, underscores or hyphens, but mustn't have a space or hyphen as the first
   * character. Any given app should have no more than ~300 distinct event names. * @param
   * eventName
   ...
   * include spaces, underscores or hyphens, but mustn't have a space or hyphen as the first
   * character. Any given app should have no more than ~300 distinct event names.
   * @param parameters A Bundle of parameters to log with the event. Insights will allow
   * looking at
   * the logs of these events via different parameter values. You can log on the order of 10
   * parameters with each distinct eventName. It's advisable to keep the number of unique values
   * provided for each parameter in the, at most, thousands. As an example, don't attempt to
   * provide
   * a unique parameter value for each unique user in your app. You won't get meaningful aggregate
   * reporting on so many parameter values. The values in the bundles should be Strings or numeric
   * values.
   * @beta
   */
  ...
  public enum FlushBehavior {
    ...
    /**
     * Flush automatically: periodically (once a minute or after every 100 events), and always at
  ...
  private enum SuppressionTimeoutBehavior {
    ...
    // Attempting to log an event, even if it is suppressed, will reset the timeout period (i.e.,
    // events will not
  ...
  // application ID and access token (which may be null for Session-less calls). This avoids
  // needing to
  ...
  private static class AccessTokenAppIdPair implements Serializable {
    ...
    public boolean equals(Object o) {
      ...
      return Utility.areObjectsEqual(p.accessToken, accessToken) && Utility.areObjectsEqual(
  ...
  static class SessionEventsState {
    ...
    public synchronized List<AppEvent> getEventsToPersist() {
      // We will only persist accumulated events, not ones currently in-flight. This means if an
      // in-flight
      // request fails, those requests will not be persisted and thus might be lost if the
      // process terminates
  ...
  static class AppEvent implements Serializable {
    ...
    public AppEvent(String eventName, Double valueToSum, Bundle parameters,
      ...
      try {
        ...
        if (!isImplicit) {
          Logger.log(LoggingBehavior.APP_EVENTS, "AppEvents", "Created app event '%s'",
              jsonObject.toString());
        }
  ...
  // Read/write operations are thread-safe/atomic across all instances of PersistedEvents,
  // but modifications
  ...
  static class PersistedEvents {
    ...
    public static void persistEvents(Context context, AccessTokenAppIdPair accessTokenAppId,
      ...
      Map<AccessTokenAppIdPair, SessionEventsState> map =
          new HashMap<AccessTokenAppIdPair, SessionEventsState>();
    ...
    public static void persistEvents(Context context,
      ...
      synchronized (staticLock) {
        ...
        // SessionEventsState; when a particular Context is being destroyed,
        // we'll persist all accumulated
        // events. More sophisticated tracking could be done to try to reduce unnecessary
    ...
    private void write() {
      ...
      try {
        oos = new ObjectOutputStream(new BufferedOutputStream(context.openFileOutput(
    ...
    private void readAndClearStore() {
      ...
      try {
        ois = new ObjectInputStream(new BufferedInputStream(context.openFileInput(
            PERSISTED_EVENTS_FILENAME)));

        @SuppressWarnings("unchecked") HashMap<AccessTokenAppIdPair, List<AppEvent>> obj =
