...
/**
 * <p> The AppEventsLogger class allows the developer to log various types of
 * events back to Facebook. In order to log events, the app must create an
 * instance of this class via a {@link #newLogger newLogger} method, and then
 * call the various "log" methods off of that. </p> <p> This client-side event
 * logging is then available through Facebook App Insights and for use with
 * Facebook Ads conversion tracking and optimization. </p> <p> The
 * AppEventsLogger class has a few related roles: <ul> <li> Logging predefined
 * and application-defined events to Facebook App Insights with a numeric value
 * to sum across a large number of events, and an optional set of key/value
 * parameters that define "segments" for this event (e.g., 'purchaserStatus' :
 * 'frequent', or 'gamerLevel' : 'intermediate'). These events may also be used
 * for ads conversion tracking, optimization, and other ads related targeting in
 * the future. </li> <li> Methods that control the way in which events are
 * flushed out to the Facebook servers. </li> </ul> Here are some important
 * characteristics of the logging mechanism provided by AppEventsLogger: <ul>
 * <li> Events are not sent immediately when logged. They're cached and flushed
 * out to the Facebook servers in a number of situations: <ul> <li>when an event
 * count threshold is passed (currently 100 logged events).</li> <li>when a time
 * threshold is passed (currently 60 seconds).</li> <li>when an app has gone to
 * background and is then brought back to the foreground.</li> </ul> <li> Events
 * will be accumulated when the app is in a disconnected state, and sent when
 * the connection is restored and one of the above 'flush' conditions are met.
 * </li> <li> The AppEventsLogger class is intended to be used from the thread
 * it was created on. Multiple AppEventsLoggers may be created on other threads
 * if desired. </li> <li> The developer can call the setFlushBehavior method to
 * force the flushing of events to only occur on an explicit call to the `flush`
 * method. </li> <li> The developer can turn on console debug output for event
 * logging and flushing to the server
 * Settings.addLoggingBehavior(LoggingBehavior.APP_EVENTS); </li> </ul> Some
 * things to note when logging events: <ul> <li> There is a limit on the number
 * of unique event names an app can use, on the order of 300. </li> <li> There
 * is a limit to the number of unique parameter names in the provided parameters
 * that can be used per event, on the order of 10. This is not just for an
 * individual call, but for all invocations for that eventName. </li> <li> Event
 * names and parameter names (the keys in the NSDictionary) must be between 2
 * and 40 characters, and must consist of alphanumeric characters, _, -, or
 * spaces. </li> <li> The length of each parameter value can be no more than on
 * the order of 100 characters. </li> </ul>
 *
 * @beta
 */
public class AppEventsLogger {
    ...
    /**
     * Sets whether events sent to Facebook should be restricted from being used
     * for purposes than analytics and conversions, such as remarketing for this
     * user. Defaults to false. This value is stored on the device and persists
     * across app launches.
     *
     ...
     * for purposes than analytics and conversions, such as remarketing for this
     * user. Defaults to false. This value is stored on the device and persists
     * across app launches. Changes to this setting will apply to any events
     * currently queued to be flushed, as well as any subsequent events.
     *
     ...
     *
     * @param context Used to access the applicationId and the attributionId for
     ...
     * app.
     *
     * @param context Used to access the attributionId for non-authenticated
     * users.
     *
     * @param applicationId The specific applicationId to report the activation
     * for.
     *
     * @beta
     */
     ...
     * @param context Used to access the attributionId for non-authenticated
     * users.
     * @param session Explicitly specified Session to log events against. If
     * null, the activeSession will be used if it's open, otherwise the logging
     * will happen against the default app ID specified via the app ID specified
     ...
     * @param context Used to access the attributionId for non-authenticated
     * users.
     * @param applicationId Explicitly specified Facebook applicationId to log
     * events against. If null, the default app ID specified in the package
     * metadata will be used.
     * @param session Explicitly specified Session to log events against. If
     * null, the activeSession will be used if it's open, otherwise the logging
     * will happen against the specified app ID.
     *
     * @return AppEventsLogger instance to invoke log* methods on.
     *
     * @beta
     ...
     *
     * @param eventName eventName used to denote the event. Choose amongst the
     * EVENT_NAME_* constants in {@link AppEventsConstants} when possible. Or
     * create your own if none of the EVENT_NAME_* constants are applicable.
     * Event names should be 40 characters or less, alphanumeric, and can
     * include spaces, underscores or hyphens, but mustn't have a space or
     * hyphen as the first character. Any given app should have no more than
     * ~300 distinct event names.
     *
     * @beta
     */
     ...
     * ~300 distinct event names.
     *                  * @param eventName
     * @param valueToSum a value to associate with the event which will be
     * summed up in Insights for across all instances of the event, so that
     ...
     * hyphen as the first character. Any given app should have no more than
     * ~300 distinct event names.
     * @param parameters A Bundle of parameters to log with the event. Insights
     * will allow looking at the logs of these events via different parameter
     * values. You can log on the order of 10 parameters with each distinct
     * eventName. It's advisable to keep the number of unique values provided
     * for each parameter in the, at most, thousands. As an example, don't
     * attempt to provide a unique parameter value for each unique user in your
     * app. You won't get meaningful aggregate reporting on so many parameter
     * values. The values in the bundles should be Strings or numeric values.
     *
     * @beta
     */
     ...
     * specified currency.
     *
     * @param purchaseAmount Amount of purchase, in the currency specified by
     * the 'currency' parameter. This value will be rounded to the thousandths
     * place (e.g., 12.34567 becomes 12.346).
     ...
     * place (e.g., 12.34567 becomes 12.346).
     * @param currency Currency used to specify the amount.
     * @param parameters Arbitrary additional information for describing this
     * event. Should have no more than 10 entries, and keys should be mostly
